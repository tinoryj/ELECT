...
public abstract class SSTableReaderBuilder {
     ...
     * @param summaryLoaded       true if index summary, first key and last key are
     *                            already loaded and not need to build again
     */
        ...
        try (RandomAccessReader primaryIndex = RandomAccessReader
            ...
            try (IndexSummaryBuilder summaryBuilder = summaryLoaded ? null
                ...
                while ((indexPosition = primaryIndex.getFilePointer()) != indexSize) {
                    ...
                    // if summary was already read from disk we don't want to re-populate it using
                    // primary index
                    if (!summaryLoaded) {
    ...
    public static class ForRead extends SSTableReaderBuilder {
        ...
        public SSTableReader build() {
            ...
            try {
                ...
                long start = nanoTime();
                load(validationMetadata, isOffline, components, DatabaseDescriptor.getDiskOptimizationStrategy(),
                        statsMetadata);
        ...
        private void load(ValidationMetadata validation,
            ...
            if (metadata.params.bloomFilterFpChance == 1.0) {
                ...
                bf = FilterFactory.AlwaysPresent;
            } else if (!components.contains(Component.PRIMARY_INDEX)) // What happens if filter component and primary
                                                                      // index is missing?
            {
        ...
        /**
         * Loads ifile, dfile and indexSummary, and optionally recreates and persists
         * the bloom filter.
         * 
         * @param recreateBloomFilter  Recreate the bloomfilter.
         * @param saveSummaryIfCreated for bulk loading purposes, if the summary was
         *                             absent and needed to be built, you can
         *                             avoid persisting it to disk by setting this to
         *                             false
         */
