 ...
 * either
 * read from disk at startup, or constructed from a flushed memtable, or after
 * compaction
 * to replace some existing sstables. However once created, an sstablereader may
 * also be modified.
 *
 ...
 * TMPLINK and FINAL; the latter corresponds
 * to NORMAL state readers and all readers that replace a NORMAL one. TMPLINK is
 * used for EARLY state readers and
 * no others.
 *
 * When a reader is being compacted, if the result is large its replacement may
 * be opened as EARLY before compaction
 * completes in order to present the result to consumers earlier. In this case
 * the reader will itself be changed to
 * a MOVED_START state, where its start no longer represents its on-disk minimum
 * key. This is to permit reads to be
 * directed to only one reader when the two represent the same data. The EARLY
 * file can represent a compaction result
 * that is either partially complete and still in-progress, or a complete and
 * immutable sstable that is part of a larger
 ...
 * state prior to completion, regardless
 * of if early opening is enabled.
 *
 * Since a reader can be created multiple times over the same shared underlying
 * resources, and the exact resources
 * it shares between each instance differ subtly, we track the lifetime of any
 * underlying resource with its own
 * reference count, which each instance takes a Ref to. Each instance then
 * tracks references to itself, and once these
 ...
 * a certain stage of their lifecycle
 * (i.e. EARLY or NORMAL opening), and some that must only occur once all
 * readers of any kind over a single logical
 * sstable have expired. These are managed by the TypeTidy and GlobalTidy
 * classes at the bottom, and are effectively
 * managed as another resource each instance tracks its own Ref instance to, to
 * ensure all of these resources are
...
public abstract class SSTableReader extends SSTable implements UnfilteredSource, SelfRefCounted<SSTableReader> {
    ...
    private static final boolean TRACK_ACTIVITY = CassandraRelevantProperties.DISABLE_SSTABLE_ACTIVITY_TRACKING
            .getBoolean();
    ...
    public static final Comparator<SSTableReader> maxTimestampDescending = (o1, o2) -> Long
            .compare(o2.getMaxTimestamp(), o1.getMaxTimestamp());
    public static final Comparator<SSTableReader> maxTimestampAscending = (o1, o2) -> Long.compare(o1.getMaxTimestamp(),
            o2.getMaxTimestamp());

    // it's just an object, which we use regular Object equality on; we introduce a
    // special class just for easy recognition
    public static final class UniqueIdentifier {
    }

    public static final Comparator<SSTableReader> sstableComparator = (o1, o2) -> o1.first.compareTo(o2.first);

    public static final Comparator<SSTableReader> idComparator = Comparator.comparing(t -> t.descriptor.id,
    ...
    /**
     * maxDataAge is a timestamp in local server time (e.g. Global.currentTimeMilli)
     * which represents an upper bound
     ...
     * When a sstable is created from compaction, maxDataAge is set to max of all
     * merged sstables.
     *
    ...
    // but it seems like a good extra layer of protection against reference counting
    // bugs to not delete data based on that alone
     ...
     * If cardinality estimator is available on all given sstables, then this method
    ...
    public static long getApproximateKeyCount(Iterable<SSTableReader> sstables) {
        ...
        for (SSTableReader sstable : sstables) {
            ...
            try {
                ...
                // If we can't load the CompactionMetadata, we are forced to estimate the keys
                // using the index
        ...
        // if something went wrong above or cardinality is not available, calculate
        // using index summary
        if (count < 0) {
            count = 0;
            for (SSTableReader sstable : sstables)
    ...
    /**
     * Estimates how much of the keys we would keep if the sstables were compacted
     * together
     */
    ...
    public static SSTableReader openECSSTable(ECMetadata ecMetadata, String sstHash, ColumnFamilyStore cfs, String fileNamePrefix) throws IOException {
        ...
        if(ecSSTable.getSSTableHashID().equals(sstHash) || sstHash == null) {
            ...
            logger.debug("rymDebug: map sstHash ({}) to ecSSTable ({}) for ecMetadata ({})", ecSSTable.getSSTableHashID(), ecSSTable.descriptor, ecMetadata.stripeId);
        } else {
    ...
    public static SSTableReader openForBatch(Descriptor descriptor, Set<Component> components,
        ...
        if (validationMetadata != null && !partitionerName.equals(validationMetadata.partitioner)) {
            logger.error(
                    "Cannot open {}; partitioner {} does not match system partitioner {}.  Note that the default partitioner starting with Cassandra 1.2 is Murmur3Partitioner, so you will need to edit that to match your old partitioner if upgrading.",
                    descriptor, validationMetadata.partitioner, partitionerName);
            System.exit(1);
        }

        try {
            return new SSTableReaderBuilder.ForBatch(descriptor, metadata, components, statsMetadata,
     ...
     * @param isOffline  Whether we are opening this SSTable "offline", for example
     *                   from an external tool or not for inclusion in queries
     *                   (validations)
     *                   This stops regenerating BF + Summaries and also disables
     *                   tracking of hotness for the SSTable.
    ...
    public static SSTableReader internalOpen(Descriptor desc,
        ...
        assert desc != null && ifile != null && dfile != null && summary != null && bf != null
                && sstableMetadata != null;

        return new SSTableReaderBuilder.ForWriter(desc, metadata, maxDataAge, components, sstableMetadata, openReason,
                header)
     ...
     * @throws CorruptSSTableException, if TOC expects compression info but not
     *                                  found from disk.
        ...
        if (tocFile.exists()) {
            try {
                ...
                if (expectedComponents.contains(Component.COMPRESSION_INFO)
                    ...
                    throw new CorruptSSTableException(new NoSuchFileException(compressionInfoFileName),
                            compressionInfoFileName);
                }
            } catch (IOException e) {
    ...
    /**
     * Execute provided task with sstable lock to avoid racing with index summary
     * redistribution, SEE CASSANDRA-15861.
     *
    ...
    // These runnables must NOT be an anonymous or non-static inner class, nor must
    ...
    /**
     * Clone this reader with the provided start and open reason, and set the clone
     * as replacement.
     *
     * @param newFirst the first key for the replacement (which can be different
     *                 from the original due to the pre-emptive
    ...
    // runOnClose must NOT be an anonymous or non-static inner class, nor must it
    // retain a reference chain to this reader
    ...
    /**
     * Returns a new SSTableReader with the same properties as this SSTableReader
     * except that a new IndexSummary will
     * be built at the target samplingLevel. This (original) SSTableReader instance
     * will be marked as replaced, have
     * its DeletingTask removed, and have its periodic read-meter sync task
     * cancelled.
     * 
     * @param samplingLevel the desired sampling level for the index summary on the
     *                      new SSTableReader
    ...
    public SSTableReader cloneWithNewSummarySamplingLevel(ColumnFamilyStore parent, int samplingLevel)
        ...
        if (samplingLevel > indexSummary.getSamplingLevel() || indexSummary.getMinIndexInterval() != minIndexInterval
                || effectiveInterval > maxIndexInterval) {
        ...
        // Always save the resampled index with lock to avoid racing with entire-sstable
        // streaming
        synchronized (tidy.global) {
    ...
    private IndexSummary buildSummaryAtLevel(int newSamplingLevel) throws IOException {
        // we read the positions in a BRAF so we don't have to worry about an entry
        // spanning a mmap boundary.
    ...
    private void validate() {
        if (this.first.compareTo(this.last) > 0) {
            throw new CorruptSSTableException(new IllegalStateException(
                    String.format("SSTable first key %s > last key %s", this.first, this.last)), getFilename());
        }
    }

    /**
     * Gets the position in the index file to start scanning to find the given key
     * (at most indexInterval keys away,
     * modulo downsampling of the index summary). Always returns a
     * {@code value >= 0}
     */
    ...
    /**
     * @return An estimate of the number of keys in this SSTable based on the index
     *         summary.
     */
    ...
    /**
     * Returns the number of entries in the IndexSummary. At full sampling, this is
     * approximately 1/INDEX_INTERVALth of
    ...
    /**
     * Returns the approximate number of entries the IndexSummary would contain if
     * it were at full sampling.
     */
    ...
    private static List<IndexesBounds> getSampleIndexesForRanges(IndexSummary summary,
        ...
        for (Range<Token> range : Range.normalize(ranges)) {
            ...
            if (right < 0) {
                // range are end inclusive so we use the previous index from what binarySearch
                // give us
    ...
    /**
     * Determine the minimal set of sections that can be extracted from this SSTable
     * to cover the given ranges.
     * 
     * @return A sorted list of (offset,end) pairs that cover the given ranges in
     *         the datafile for this SSTable.
     */
     ...
     * 
     * @param key The key to apply as the rhs to the given Operator. A 'fake' key is
     *            allowed to
     *            allow key selection by token bounds but only if op != * EQ
     * @param op  The Operator defining matching keys: the nearest key to the target
     *            matching the operator wins.
     */
    ...
    /**
     * @param key                 The key to apply as the rhs to the given Operator.
     *                            A 'fake' key is allowed to
     *                            allow key selection by token bounds but only if op
     *                            != * EQ
     * @param op                  The Operator defining matching keys: the nearest
     *                            key to the target matching the operator wins.
     * @param updateCacheAndStats true if updating stats and cache
     * @param listener            a listener used to handle internal events
     * @return The index entry corresponding to the key, or null if the key is not
     *         present
     */
    ...
    public abstract UnfilteredRowIterator rowIterator(FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry,
            Slices slices, ColumnFilter selectedColumns, boolean reversed);

    public abstract UnfilteredRowIterator simpleIterator(FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry,
            boolean tombstoneOnly);

    /**
     * Finds and returns the first key beyond a given token in this SSTable or null
     ...
     *
     * When calling this function, the caller must ensure that the SSTableReader is
     ...
     * This works in conjunction with maxDataAge which is an upper bound on the
     * create of data in this sstable.
     * 
     * @param age The age to compare the maxDataAre of this sstable. Measured in
     *            millisec since epoc on this host
     * @return True iff this sstable contains data that's newer than the given age
     *         parameter.
     */
    ...
    public DecoratedKey keyAt(long indexPosition) throws IOException {
        ...
        try (FileDataInput in = ifile.createReader(indexPosition)) {
            ...
            // this saves index summary lookup and index file iteration which whould be
            // pretty costly
    ...
    public abstract static class Operator {
        ...
        /**
         * @param comparison The result of a call to compare/compareTo, with the desired
         *                   field on the rhs.
         * @return less than 0 if the operator cannot match forward, 0 if it matches,
    ...
    /**
     * Whether the sstable may contain tombstones or if it is guaranteed to not
     * contain any.
     * <p>
     * Note that having that method return {@code false} guarantees the sstable has
     * no tombstones whatsoever (so no
     * cell tombstone, no range tombstone maker and no expiring columns), but having
     * it return {@code true} doesn't
     * guarantee it contains any as it may simply have non-expired cells.
     */
    public boolean mayHaveTombstones() {
        // A sstable is guaranteed to have no tombstones if minLocalDeletionTime is
        // still set to its default,
    ...
    /**
     * Mutate sstable level with a lock to avoid racing with
    ...
    /**
     * Increment the total read count and read rate for this SSTable. This should
     * not be incremented for non-query reads,
     * like compaction.
     */
    ...
    public EncodingStats stats() {
        // We could return sstable.header.stats(), but this may not be as accurate than
        // the actual sstable stats (see
     ...
     *
     * We can create many InstanceTidiers (one for every time we reopen an sstable
     * with MOVED_START for example),
     ...
     * GlobalTidy; when all InstanceTidiers
    ...
    /**
     * One instance per logical sstable. This both tracks shared cleanup and some
     * shared state related
     * to the sstable's lifecycle.
     *
     * All InstanceTidiers, on setup(), ask the static get() method for their shared
     * state,
     * and stash a reference to it to be released when they are. Once all such
     * references are
    ...
    static final class GlobalTidy implements Tidy {
        ...
        // the readMeter that is shared between all instances of the sstable, and can be
        // overridden in all of them
        // at once also, for testing purposes
        private RestorableMeter readMeter;
        // the scheduled persistence of the readMeter, that we will cancel once all
        ...
        void ensureReadMeter() {
            ...
            // Also, do not track read rates when running in client or tools mode
            // (syncExecuter isn't available in these modes)
            ...
            // sync the average read rate to system.sstable_activity every five minutes,
            // starting one minute from now
    ...
    /**
     * Moves the sstable in oldDescriptor to a new place (with generation etc) in
     * newDescriptor.
     *
     * All components given will be moved/renamed
     */
    public static SSTableReader moveAndOpenSSTable(ColumnFamilyStore cfs, Descriptor oldDescriptor,
            Descriptor newDescriptor, Set<Component> components, boolean copyData) {
        ...
        if (isLive) {
            String message = String.format("Can't move and open a file that is already in use in the table %s -> %s",
        ...
        if (copyData) {
            try {
                ...
                logger.warn("Unable to hardlink new SSTable {} to {}, falling back to copying", oldDescriptor,
                        newDescriptor, ex);
