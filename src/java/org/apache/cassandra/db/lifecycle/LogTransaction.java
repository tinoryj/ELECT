...
/**
 * IMPORTANT: When this object is involved in a transactional graph, and is not
 * encapsulated in a LifecycleTransaction,
 * for correct behaviour its commit MUST occur before any others, since it may
 * legitimately fail. This is consistent
 * with the Transactional API, which permits one failing action to occur at the
 * beginning of the commit phase, but also
 * *requires* that the prepareToCommit() phase only take actions that can be
 * rolled back.
 *
 * IMPORTANT: The transaction must complete (commit or abort) before any
 * temporary files are deleted, even though the
 * txn log file itself will not be deleted until all tracked files are deleted.
 * This is required by FileLister to ensure
 * a consistent disk state. LifecycleTransaction ensures this requirement, so
 * this class should really never be used
 ...
 * if the transaction succeeds the old files should be deleted and the new ones
 * kept; vice-versa if it fails.
 *
 ...
 *
 * where sstable-2 is a new sstable to be retained if the transaction succeeds
 * and sstable-1 is an old sstable to be
 * removed. CRC is an incremental CRC of the file content up to this point. For
 * old sstable files we also log the
 * last update time of all files for the sstable descriptor and the number of
 * sstable files.
 *
 ...
 *
 * When the transaction log is cleaned-up by the TransactionTidier, which
 * happens only after any old sstables have been
 * osoleted, then any sstable files for old sstables are removed before deleting
 * the transaction log if the transaction
 * was committed, vice-versa if the transaction was aborted.
 *
 * On start-up we look for any transaction log files and repeat the cleanup
...
class LogTransaction extends Transactional.AbstractTransactional implements Transactional {
    ...
    // We need an explicit lock because the transaction tidier cannot store a
     ...
     *
     * When the transaction reference is fully released we try to delete all the
     * obsolete files
    ...
    private static class TransactionTidier implements RefCounted.Tidy, Runnable {
        ...
        public void run() {
            synchronized (lock) {
                ...
                // this happens if we forget to close a txn and the garbage collector closes it
                // for us
                ...
                if (err != null) {
                    logger.info(
                            "Failed deleting files for transaction {}, we'll retry after GC and on on server restart",
                            name(), err);
    ...
    /**
     * The SSTableReader tidier. When a reader is fully released and no longer
     * referenced
     * by any one, we run this. It keeps a reference to the parent transaction and
     * releases
     * it when done, so that the final transaction cleanup can run when all obsolete
     * readers
     * are released.
     */
    public static class SSTableTidier implements Runnable {
        // must not retain a reference to the SSTableReader, else leak detection cannot
        // kick in
        ...
        public SSTableTidier(SSTableReader referent, boolean wasNew, LogTransaction parent) {
            ...
            // While the parent cfs may be dropped in the interim of us taking a reference
            // to this and using it, at worst
            // we'll be updating a metric for a now dropped ColumnFamilyStore. We do not
            // hold a reference to the tracker or
            // cfs as that would create a strong ref loop and violate our ability to do leak
            // detection.
        ...
        public void run() {
            // While this may be a dummy tracker w/out information in the metrics table, we
            // attempt to delete regardless
            // and allow the delete to silently fail if this is an invalid ks + cf
            // combination at time of tidy run.
            if (DatabaseDescriptor.isDaemonInitialized())
                SystemKeyspace.clearSSTableReadMeter(desc.ksname, desc.cfname, desc.id);

            synchronized (lock) {
                try {
                    // If we can't successfully delete the DATA component, set the task to be
                    // retried later: see TransactionTidier
                    if (new File(desc.filenameFor(Component.DATA)).exists()) {
                        ...
                        File ecMetasdataFile = new File(desc.filenameFor(Component.EC_METADATA));

                        if (logger.isTraceEnabled())
                            logger.trace("Tidier running for old sstable {}", desc.baseFilename());

                        if (ecMetasdataFile.exists())
                            delete(ecMetasdataFile);
                        else if (!wasNew)
                            logger.error(
                                    "SSTableTidier ran with no existing data file for an sstable that was not new");
                    }
                ...
                // It's possible we're the last one's holding a ref to this metric if it's
                // already been released in the
                // parent TableMetrics; we run this regardless rather than holding a ref to that
                // CFS or Tracker and thus
                ...
                // release the referent to the parent so that the all transaction files can be
                // released
                parentRef.release();
            }
        }
    ...
    /**
     * Removes any leftovers from unifinished transactions as indicated by any
     * transaction log files that
     * are found in the table directories. This means that any old sstable files for
     * transactions that were committed,
     * or any new sstable files for transactions that were aborted or still in
     * progress, should be removed *if
     * it is safe to do so*. Refer to the checks in LogFile.verify for further
     * details on the safety checks
     ...
     *
     * This method is called on startup and by the standalone sstableutil tool when
     * the cleanup option is specified,
     * 
     * @see org.apache.cassandra.tools.StandaloneSSTableUtil
     *
     * @return true if the leftovers of all transaction logs found were removed,
     *         false otherwise.
     *
     */
    ...
    private static final class LogFilesByName {
        // This maps a transaction log file name to a list of physical files. Each
        // sstable
        // can have multiple directories and a transaction is trakced by identical
        // transaction log
        // files, one per directory. So for each transaction file name we can have
        // multiple
        // physical files.
