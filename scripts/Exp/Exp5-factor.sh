...
function load {
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        ...
        scp -r elect@node$i:/home/elect/Results ${PathToELECTLog}/${targetScheme}/${ExpName}-TSS-${storageSaving}-Load-Node$i
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        for ((i = 1; i <= NodeNumber; i++)); do
            ...
            scp -r elect@node$i:/home/elect/Results ${PathToELECTLog}/"${targetScheme}"/"${ExpName}-TSS-${storageSaving}-${workload}-${round}-Node$i"
            ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
        done
    done
}
