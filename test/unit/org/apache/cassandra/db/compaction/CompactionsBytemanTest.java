...
public class CompactionsBytemanTest extends CQLTester {
    /*
     * Return false for the first time hasAvailableDiskSpace is called. i.e first
     * SSTable is too big
    ...
    @Test
    @BMRules(rules = {
            @BMRule(name = "One SSTable too big for remaining disk space test", targetClass = "Directories", targetMethod = "hasAvailableDiskSpace", condition = "not flagged(\"done\")", action = "flag(\"done\"); return false;") })
    ...
    @Test
    @BMRules(rules = {
            @BMRule(name = "No disk space with expired SSTables test", targetClass = "Directories", targetMethod = "hasAvailableDiskSpace", action = "return false;") })
    ...
    /*
     * Always return false for hasAvailableDiskSpace. i.e node has no more space
     * Create 2 SSTables. Compaction will not succeed and will throw Runtime
     * Exception
     */
    ...
    @Test
    @BMRule(name = "Delay background compaction task future check", targetClass = "CompactionManager", targetMethod = "submitBackground", targetLocation = "AT INVOKE java.util.concurrent.Future.isCancelled", condition = "!$cfs.keyspace.getName().contains(\"system\")", action = "Thread.sleep(5000)")
    ...
    private void createLowGCGraceTable() {
        createTable(
                "CREATE TABLE %s (id int PRIMARY KEY, val text) with compaction = {'class':'SizeTieredCompactionStrategy', 'enabled': 'false'} AND gc_grace_seconds=0");
    }

    @Test
    @BMRule(name = "Stop all compactions", targetClass = "CompactionTask", targetMethod = "runMayThrow", targetLocation = "AT INVOKE getCompactionAwareWriter", action = "$ci.stop()")
    public void testStopUserDefinedCompactionRepaired() throws Throwable {
        testStopCompactionRepaired((cfs) -> {
            Collection<Descriptor> files = cfs.getLiveSSTables().stream().map(s -> s.descriptor)
    ...
    public void testStopCompactionRepaired(Consumer<ColumnFamilyStore> compactionRunner) throws Throwable {
        ...
        assertTrue(CompactionManager.instance.active.getCompactions().stream()
                .noneMatch(h -> h.getCompactionInfo().getTableMetadata().equals(cfs.metadata)));
