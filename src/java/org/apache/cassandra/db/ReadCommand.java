...
public abstract class ReadCommand extends AbstractReadQuery {
    private static final int TEST_ITERATION_DELAY_MILLIS = Integer
            .parseInt(System.getProperty("cassandra.test.read_iteration_delay_ms", "0"));
     ...
     *
     * @return the requested digest version if the query is a digest. Otherwise,
     *         this can return
     *         anything.
     */
     ...
     * <p>
     * Note that we allow setting this independently of setting the command as a
     * digest query as
     * this allows us to use the command as a carrier of the digest version even if
     * we only call
     * setIsDigestQuery on some copy of it.
     *
     * @param digestVersion the version for the digest is this command is used for
     *                      digest query..
    ...
    /**
     * @return Whether this query expects only a transient data response, or a full
     *         response
     */
     ...
     * <p>
     * Note that that method should only be called on a key actually queried by this
     * command
     * and in practice, this will almost always return the same filter, but for the
     * sake of
     * paging, the filter on the first key of a range command might be slightly
     * different.
     *
     * @param key a partition key queried by this command.
     *
     * @return the {@code ClusteringIndexFilter} to use for the partition of key
     *         {@code key}.
     */
    ...
    public ReadCommand copyAsTransientQuery(Iterable<Replica> replicas) {
        if (any(replicas, Replica::isFull))
            throw new IllegalArgumentException("Can't make a transient request on full replicas: "
                    + Iterables.toString(filter(replicas, Replica::isFull)));
        return copyAsTransientQuery();
    }
    ...
    public ReadCommand copyAsDigestQuery(Iterable<Replica> replicas) {
        if (any(replicas, Replica::isTransient))
            throw new IllegalArgumentException("Can't make a digest request on a transient replica "
                    + Iterables.toString(filter(replicas, Replica::isTransient)));
     ...
     *
     * @return whether the underlying {@code ClusteringIndexFilter} is reversed or
     *         not.
     */
    ...
    public ReadResponse createResponse(UnfilteredPartitionIterator iterator, RepairedDataInfo rdi) {
        // validate that the sequence of RT markers is correct: open is followed by
        // close, deletion times for both
     ...
     */
    @SuppressWarnings("resource") // The result iterator is closed upon exceptions (we know it's fine to
                                  // potentially not close the intermediary
                                  // iterators created inside the try as long as we do close the original
                                  // resultIterator), or by closing the result.
    public UnfilteredPartitionIterator executeLocally(ReadExecutionController executionController) {
        ...
        try {
            ...
            try {
                ...
                iterator = RTBoundValidator.validate(withoutPurgeableTombstones(iterator, cfs, executionController),
                        Stage.PURGED, false);
                ...
                /*
                 * TODO: We'll currently do filtering by the rowFilter here because it's
                 * convenient. However,
                 * we'll probably want to optimize by pushing it down the layer (like for
                 * dropped columns) as it
                ...
                // apply the limits/row counter; this transformation is stopping and would close
                // the iterator as soon
                // as the count is observed; if that happens in the middle of an open RT, its
                // end bound will not be included.
                // If tracking repaired data, the counter is needed for overreading repaired
                // data, otherwise we can
                // optimise the case where this.limit = DataLimits.NONE which skips an
                // unnecessary transform
                if (executionController.isTrackingRepairedStatus()) {
                    DataLimits.Counter limit = limits().newCounter(nowInSec(), false, selectsFullPartition(),
                            metadata().enforceStrictLiveness());
                    iterator = limit.applyTo(iterator);
                    // ensure that a consistent amount of repaired data is read on each replica.
                    // This causes silent
                    // overreading from the repaired data set, up to limits(). The extra data is not
                    // visible to
                ...
                // because of the above, we need to append an aritifical end bound if the source
                // iterator was stopped short by a counter.
    ...
    /**
     * Wraps the provided iterator so that metrics on what is scanned by the command
     * are recorded.
     * This also log warning/trow TombstoneOverwhelmingException if appropriate.
     */
    private UnfilteredPartitionIterator withMetricsRecording(UnfilteredPartitionIterator iter,
            final TableMetrics metric, final long startTimeNanos) {
        class MetricRecording extends Transformation<UnfilteredRowIterator> {
            private final int failureThreshold = DatabaseDescriptor.getTombstoneFailureThreshold();
            private final int warningThreshold = DatabaseDescriptor.getTombstoneWarnThreshold();

            private final boolean respectTombstoneThresholds = !SchemaConstants
                    .isLocalSystemKeyspace(ReadCommand.this.metadata().keyspace);
            ...
            private void countTombstone(ClusteringPrefix<?> clustering) {
                ...
                if (tombstones > failureThreshold && respectTombstoneThresholds) {
                    ...
                    Tracing.trace(
                            "Scanned over {} tombstones for query {}; query aborted (see tombstone_failure_threshold)",
                    ...
                    throw new TombstoneOverwhelmingException(tombstones, query, ReadCommand.this.metadata(), currentKey,
                            clustering);
                }
            }
    ...
    protected class CheckForAbort extends StoppingTransformation<UnfilteredRowIterator> {
        ...
        private boolean maybeAbort() {
            /**
             * TODO: this is not a great way to abort early; why not expressly limit checks
             * to 10ms intervals?
             * The value returned by approxTime.now() is updated only every
             * {@link org.apache.cassandra.utils.MonotonicClock.SampledClock.CHECK_INTERVAL_MS},
             * by default 2 millis. Since MonitorableImpl
             * relies on approxTime, we don't need to check unless the approximate time has
             * elapsed.
             */
    ...
    private boolean shouldTrackSize(DataStorageSpec.LongBytesBound warnThresholdBytes,
            DataStorageSpec.LongBytesBound abortThresholdBytes) {
        return trackWarnings
        ...
        class QuerySizeTracking extends Transformation<UnfilteredRowIterator> {
            ...
            private void addSize(long size) {
                ...
                if (failBytes != -1 && this.sizeInBytes >= failBytes) {
                    String msg = String.format(
                            "Query %s attempted to read %d bytes but max allowed is %s; query aborted  (see local_read_size_fail_threshold)",
    ...
    // Skip purgeable tombstones. We do this because it's safe to do (post-merge of
    // the memtable and sstable at least), it
    ...
    InputCollector<UnfilteredRowIterator> iteratorsForPartition(ColumnFamilyStore.ViewFragment view,
        ...
        // The merged & repaired row iterator will be consumed until it's exhausted or
        // the RepairedDataInfo's
    ...
    /**
     * Handles the collation of unfiltered row or partition iterators that comprise
     * the
     * input for a query. Separates them according to repaired status and of
     * repaired
     * status is being tracked, handles the merge and wrapping in a digest generator
     * of
    ...
    static class InputCollector<T extends AutoCloseable> {
        ...
        InputCollector(ColumnFamilyStore.ViewFragment view,
            ...
            if (repairedSSTables == null) {
                ...
                unrepairedIters = new ArrayList<>(
                        (view.sstables.size() - repairedSSTables.size()) + Iterables.size(view.memtables) + 1);
            }
        ...
        // * marked repaired
        // * marked pending, but the local session has been committed. This reduces the
        // window
        // whereby the tracking is affected by compaction backlog causing repaired
        // sstables to
        // remain in the pending state
        // If an sstable is involved in a pending repair which is not yet committed, we
        // mark the
        // repaired data info inconclusive, as the same data on other replicas may be in
        // a
        ...
        private boolean considerRepairedForTracking(SSTableReader sstable) {
            ...
            if (pendingRepair != ActiveRepairService.NO_PENDING_REPAIR) {
                ...
                // timeout and be purged by LocalSessions::cleanup, consider the sstable
                // unrepaired
    ...
    public static class Serializer implements IVersionedSerializer<ReadCommand> {
        ...
        private IndexMetadata deserializeIndexMetadata(DataInputPlus in, int version, TableMetadata metadata)
                throws IOException {
            try {
