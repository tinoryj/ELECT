...
public class MaxSSTableSizeWriter extends CompactionAwareWriter {
     ...
     */
    private static long getTotalWriteSize(Iterable<SSTableReader> nonExpiredSSTables, long estimatedTotalKeys,
            ColumnFamilyStore cfs, OperationType compactionType) {
        ...
        return Math
                .round(estimatedCompactionRatio * cfs.getExpectedCompactedFileSize(nonExpiredSSTables, compactionType));
    }

    protected boolean realAppend(UnfilteredRowIterator partition) {

        if(!sstableWriter.currentWriter().isChecked && sstableWriter.currentWriter().first != null &&
            ...
            sstableWriter.currentWriter().isChecked = true;
        }
    ...
    public void switchCompactionLocation(Directories.DataDirectory location) {
        ...
        SSTableWriter writer = SSTableWriter.create(
                cfs.newSSTableDescriptor(getDirectories().getLocationForDisk(sstableDirectory)),
