 ...
 * in the value to the prepared statement, which it associates with the key, and
 * in
...
class CqlRecordWriter extends RecordWriter<Map<String, ByteBuffer>, List<ByteBuffer>> implements
    ...
    CqlRecordWriter(Configuration conf) {
        ...
        try (Cluster cluster = CqlConfigHelper.getOutputCluster(ConfigHelper.getOutputInitialAddress(conf), conf)) {
            ...
            TableMetadata tableMetadata = metadata.getKeyspace(Metadata.quote(keyspace))
    ...
    private static void closeSession(Session session) {
        // Close the session to satisfy to avoid warnings for the resource not being
        // closed
        try {
            if (session != null)
    ...
    /**
     * A client that runs in a threadpool and connects to the list of endpoints for
     * a particular
     * range. Bound variables for keys in that range are sent to this client via a
     * queue.
     */
    public class RangeClient extends Thread {
        ...
        // we want the caller to know if something went wrong, so we record any
        // unrecoverable exception while writing
        // so we can throw it on the caller's stack when he calls put() again, or if
        // there are no more put calls,
        ...
        /**
         * Loops collecting cql binded variable values from the queue and sending to
         * Cassandra
         */
        ...
        public void run() {
            ...
            try {
                outer: while (run || !queue.isEmpty()) {
                    ...
                    while (true) {
                        // send the mutation to the last-used endpoint. first time through, this will
                        // NPE harmlessly.
                        if (session != null) {
                            try {
                                int i = 0;
                                ...
                                while (bindVariables != null) {
                                    ...
                                    for (int columnPosition = 0; columnPosition < bindVariables
                                            .size(); columnPosition++) {
                        ...
                        try {
                            ...
                            // If connection died due to Interrupt, just try connecting to the endpoint
                            // again.
                            // There are too many ways for the Thread.interrupted() state to be cleared, so
                            // we can't rely on that here. Until the java driver gives us a better way of
                            // knowing
                            // that this exception came from an InterruptedException, this is the best
                            // solution.
                            ...
                            // we should try again to another. Other exceptions (auth or invalid request)
                            // are fatal.
        ...
        /**
         * get prepared statement id from cache, otherwise prepare it from Cassandra
         * server
         */
        ...
        public void close() throws IOException {
            // stop the run loop. this will result in closeInternal being called by the time
            // join() finishes.
            run = false;
            interrupt();
            try {
    ...
    private String appendKeyWhereClauses(String cqlQuery) {
        ...
        for (ColumnMetadata partitionKey : partitionKeyColumns)
            keyWhereClause += String.format("%s = ?", keyWhereClause.isEmpty() ? quote(partitionKey.getName())
