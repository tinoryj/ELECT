...
function load {
    ...
    if [ ${targetScheme} == "elect" ]; then
        ...
        sed -i "s/\(data_block_num: \)".*"/data_block_num: ${coding}/" playbook-load.yaml
    else
    ...
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${targetScheme}-K-${coding}-Load"/" playbook-load.yaml
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        ...
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-K-${coding}-Load-Node$i
...
function flush {
    ...
    # Modify playbook
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${coding}-${targetScheme}-Load"/" playbook-flush.yaml
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        echo "Copy loading stats of ${targetScheme} back, node$i"
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-K-${coding}-Compact-Node$i
        ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
    done
}
...
function backup {
    ...
    sed -i "s/DATAPATH/${ExpName}-${coding}/g" playbook-backup.yaml
...
function startup {
    ...
    if [ ${targetScheme} == "elect" ]; then
        ...
        sed -i "s/\(data_block_num: \)".*"/data_block_num: ${coding}/" playbook-startup.yaml
    ...
    sed -i "s/DATAPATH/${ExpName}-${coding}/g" playbook-startup.yaml
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${targetScheme}-Run-${runningType}-K-${coding}-Round-${round}"/" playbook-run.yaml
        ...
        for ((i = 1; i <= NodeNumber; i++)); do
            ...
            scp -r elect@node$i:/home/elect/Results /home/elect/Results/"${targetScheme}"/"${ExpName}-K-${coding}-${workload}-${round}-Node$i"
            ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
        done
    done
}
...
for scheme in "${schemes[@]}"; do
    ...
    if [ "${scheme}" == "elect" ]; then
        waitTime=$(echo "scale=2; ($dataSizeOnEachNode * 1024 / 4 / 2 / ($concurrentEC / 2)) * 130 + $dataSizeOnEachNode * 450" | bc)
        maxLevelCount
    ...
    for coding in "${codingK[@]}"; do
        load "${scheme}" "${coding}"

        if [ "${waitFlush}" == "true" ]; then
            flush "${scheme}" "${waitTimeCeil}" "${coding}"
        fi

        if [ "${restart}" == "true" ]; then
            backup "${scheme}" "${coding}"
        fi
        ...
        for ((round = 1; round <= roundNumber; round++)); do
            for type in "${types[@]}"; do
                if [ "${type}" == "normal" ]; then
                    if [ "${restart}" == "true" ]; then
                        startup "${scheme}" "${coding}"
                    fi
                    run "${scheme}" "${round}" "${type}" "${coding}"
