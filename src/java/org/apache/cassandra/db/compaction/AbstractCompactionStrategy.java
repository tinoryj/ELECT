 ...
 * - perform background compaction constantly as needed; this typically makes a
 * tradeoff between
...
public abstract class AbstractCompactionStrategy {
    ...
    // minimum interval needed to perform tombstone removal compaction in seconds,
    ...
    // disable range overlap check when deciding if an SSTable is candidate for
    // tombstone compaction (CASSANDRA-6563)
    ...
    /**
     * pause/resume/getNextBackgroundTask must synchronize. This guarantees that
     * after pause completes,
     * no new tasks will be generated; or put another way, pause can't run until
     * in-progress tasks are
     * done being created.
     *
     * This allows runWithCompactionsDisabled to be confident that after pausing,
     * once in-progress
    ...
    protected AbstractCompactionStrategy(ColumnFamilyStore cfs, Map<String, String> options) {
        ...
        /*
         * checks must be repeated here, as user supplied strategies might not call
         * validateOptions directly
         */
    ...
    /**
     * For internal, temporary suspension of background compactions so that we can
     * do exceptional
    ...
    /**
     * Releases any resources if this strategy is shutdown (when the CFS is reloaded
     ...
     *
     * @return the next background/minor compaction task to run; null if nothing to
     *         do.
     *
     ...
     *         Will not be null. (Will throw if user requests an invalid
     *         compaction.)
     *
     *         Is responsible for marking its sstables as compaction-pending.
     */
    ...
    public AbstractCompactionTask getCompactionTask(LifecycleTransaction txn, final int gcBefore,
            long maxSSTableBytes) {
        return new CompactionTask(cfs, txn, gcBefore);
    }

    /**
     * @return the number of background tasks estimated to still be needed for this
     *         columnfamilystore
     */
    ...
    public ScannerList getScanners(Collection<SSTableReader> sstables, Range<Token> range) {
        return range == null ? getScanners(sstables, (Collection<Range<Token>>) null)
                : getScanners(sstables, Collections.singleton(range));
    }

    /**
     * Returns a list of KeyScanners given sstables and a range on which to scan.
     * The default implementation simply grab one SSTableScanner per-sstable, but
     ...
     *
     * Note that implementations must be able to handle duplicate notifications here
     * (that removed are already gone and
    ...
    /**
     * Adds sstable, note that implementations must handle duplicate notifications
     * here (added already being in the compaction strategy)
     */
    public abstract void addSSTable(SSTableReader added);

    /**
     * Adds sstables, note that implementations must handle duplicate notifications
     * here (added already being in the compaction strategy)
     */
    ...
    /**
     * Removes sstable from the strategy, implementations must be able to handle the
     * sstable having already been removed.
     */
    public abstract void removeSSTable(SSTableReader sstable);

    /**
     * Removes sstables from the strategy, implementations must be able to handle
     * the sstables having already been removed.
     */
    ...
    /**
     * Called when the metadata has changed for an sstable - for example if the
     * level changed
     *
     * Not called when repair status changes (which is also metadata), because this
     * results in the
     ...
     * Check is skipped if tombstone_compaction_interval time does not elapse since
     * sstable creation and returns false.
     *
    ...
    protected boolean worthDroppingTombstones(SSTableReader sstable, int gcBefore) {
        ...
        if (overlaps.isEmpty()) {
            ...
            return true;
        } else if (CompactionController.getFullyExpiredSSTables(cfs, Collections.singleton(sstable), overlaps, gcBefore)
                .size() > 0) {
            return true;
        } else {
            ...
            double remainingColumnsRatio = ((double) columns) / (sstable.getEstimatedCellPerPartitionCount().count()
    ...
    public static Map<String, String> validateOptions(Map<String, String> options) throws ConfigurationException {
        ...
        if (threshold != null) {
            try {
                ...
                if (thresholdValue < 0) {
                    throw new ConfigurationException(String.format("%s must be greater than 0, but was %f",
                            TOMBSTONE_THRESHOLD_OPTION, thresholdValue));
                }
            } catch (NumberFormatException e) {
                throw new ConfigurationException(String.format("%s is not a parsable int (base10) for %s", threshold,
                        TOMBSTONE_THRESHOLD_OPTION), e);
            }
        }
        ...
        if (interval != null) {
            try {
                ...
                if (tombstoneCompactionInterval < 0) {
                    throw new ConfigurationException(String.format("%s must be greater than 0, but was %d",
                            TOMBSTONE_COMPACTION_INTERVAL_OPTION, tombstoneCompactionInterval));
                }
            } catch (NumberFormatException e) {
                throw new ConfigurationException(String.format("%s is not a parsable int (base10) for %s", interval,
                        TOMBSTONE_COMPACTION_INTERVAL_OPTION), e);
            }
        }
        ...
        if (unchecked != null) {
            if (!unchecked.equalsIgnoreCase("true") && !unchecked.equalsIgnoreCase("false"))
                throw new ConfigurationException(String.format("'%s' should be either 'true' or 'false', not '%s'",
                        UNCHECKED_TOMBSTONE_COMPACTION_OPTION, unchecked));
        }
        ...
        if (logAll != null) {
            if (!logAll.equalsIgnoreCase("true") && !logAll.equalsIgnoreCase("false")) {
                throw new ConfigurationException(
                        String.format("'%s' should either be 'true' or 'false', not %s", LOG_ALL_OPTION, logAll));
            }
        }
        ...
        if (compactionEnabled != null) {
            if (!compactionEnabled.equalsIgnoreCase("true") && !compactionEnabled.equalsIgnoreCase("false")) {
                throw new ConfigurationException(
                        String.format("enabled should either be 'true' or 'false', not %s", compactionEnabled));
            }
        }
