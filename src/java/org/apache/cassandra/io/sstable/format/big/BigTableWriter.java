...
public class BigTableWriter extends SSTableWriter {
    ...
    private CompressionParams compressionFor(final OperationType opType) {
        ...
        if (null != compressor && opType == OperationType.FLUSH) {
            // When we are flushing out of the memtable throughput of the compressor is
            // critical as flushes,
            // especially of large tables, can queue up and potentially block writes.
            // This optimization allows us to fall back to a faster compressor if a
            // particular
            // compression algorithm indicates we should. See CASSANDRA-15379 for more
            // details.
            switch (DatabaseDescriptor.getFlushCompression()) {
                // It is relatively easier to insert a Noop compressor than to disable
                // compressed writing
                // entirely as the "compression" member field is provided outside the scope of
                // this class.
                // It may make sense in the future to refactor the ownership of the compression
                // flag so that
    ...
    /**
     * Perform sanity checks on @param decoratedKey and @return the position in the
     * data file before any data is written
     */
    ...
    private void afterAppend(DecoratedKey decoratedKey, long dataEnd, RowIndexEntry index, ByteBuffer indexInfo)
            throws IOException {
    ...
    public RowIndexEntry append(UnfilteredRowIterator iterator) {
        ...
        if (key.getKey().remaining() > FBUtilities.MAX_UNSIGNED_SHORT) {
            logger.error("Key size {} exceeds maximum of {}, skipping row", key.getKey().remaining(),
        ...
        try (UnfilteredRowIterator collecting = Transformation.apply(iterator, new StatsCollector(metadataCollector))) {
            ...
            // afterAppend() writes the partition key before the first RowIndexEntry - so we
            // have to add it's
            // serialized size to the index-writer position
            long indexFilePosition = ByteBufferUtil.serializedSizeWithShortLength(key.getKey())
    ...
    private void maybeLogLargePartitionWarning(DecoratedKey key, long rowSize) {
        if (rowSize > DatabaseDescriptor.getCompactionLargePartitionWarningThreshold()) {
            ...
            logger.warn("Writing large partition {}/{}:{} ({}) to sstable {}", metadata.keyspace, metadata.name,
                    keyString, FBUtilities.prettyPrintMemory(rowSize), getFilename());
        }
    }
    ...
    private void maybeLogManyTombstonesWarning(DecoratedKey key, int tombstoneCount) {
        if (tombstoneCount > DatabaseDescriptor.getCompactionTombstoneWarningThreshold()) {
            String keyString = metadata().partitionKeyType.getString(key.getKey());
            logger.warn("Writing {} tombstones to {}/{}:{} in sstable {}", tombstoneCount, metadata.keyspace,
                    metadata.name, keyString, getFilename());
        }
    }
    ...
    public SSTableReader openEarly() {
        ...
        try {
            ...
            int dataBufferSize = optimizationStrategy.bufferSize(stats.estimatedPartitionSize
            ...
            // now it's open, find the ACTUAL last readable key (i.e. for which the data
            // file has also been flushed)
            sstable.first = getMinimalKey(first);
            sstable.last = getMinimalKey(boundary.lastKey);
            return sstable;
        } catch (Throwable t) {
            JVMStabilityInspector.inspectThrowable(t);
            // If we successfully created our sstable, we can rely on its InstanceTidier to
    ...
    class IndexWriter extends AbstractTransactional implements Transactional {
        ...
        IndexWriter(long keyCount) {
            ...
            summary = new IndexSummaryBuilder(keyCount, metadata().params.minIndexInterval,
        ...
        public void append(DecoratedKey key, RowIndexEntry indexEntry, long dataEnd, ByteBuffer indexInfo)
                throws IOException {
            bf.add(key);
        ...
        /**
         * Closes the index and bloomfilter, making the public state of this writer
        ...
        public void resetAndTruncate() {
            // we can't un-set the bloom filter addition, but extra keys in there are
            // harmless.
            // we can't reset dbuilder either, but that is the last thing called in
            // afterappend so
