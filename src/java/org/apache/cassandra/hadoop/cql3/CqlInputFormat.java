 ...
 * or
 * 2: ConfigHelper.setInputSplitSizeInMb. InputSplit size in MB with new, more
 * precise method
 ...
 */
public class CqlInputFormat extends org.apache.hadoop.mapreduce.InputFormat<Long, Row>
    ...
    public List<org.apache.hadoop.mapreduce.InputSplit> getSplits(JobContext context) throws IOException {
        ...
        try (Cluster cluster = CqlConfigHelper.getInputCluster(inputInitialAddress, conf);
            ...
            // TODO if the job range is defined and does perfectly match tokens, then the
            // logic will be unable to get estimates since they are pre-computed
            // tokens: [0, 10, 20]
            ...
            Map<TokenRange, List<Host>> masterRangeNodes = getRangeMap(keyspace, metadata,
                    getTargetDC(metadata, inputInitialAddress));
            for (TokenRange range : masterRangeNodes.keySet()) {
                if (jobRange == null) {
                    for (TokenRange unwrapped : range.unwrap()) {
                        // for each tokenRange, pick a live owner and ask it for the byte-sized splits
                        SplitFuture task = new SplitFuture(
                                new SplitCallable(unwrapped, masterRangeNodes.get(range), conf, session));
            ...
            // The estimate is only stored on a single host, if that host is down then can
            // not get the estimate
            // its more than likely that a single host could be "too large" for one split
            ...
            if (!failedTasks.isEmpty()) {
                ...
                for (SplitFuture task : failedTasks) {
                    try {
                        ...
                        logger.warn(
                                "Unable to get estimate for {}, the host {} had a exception; falling back to default estimate",
                                task.splitCallable.tokenRange, task.splitCallable.hosts.get(0), cause);
                    }
                }
                for (SplitFuture task : failedTasks)
                    splits.addAll(toSplit(task.splitCallable.hosts, splitTokenRange(task.splitCallable.tokenRange,
                            maxSplits, expectedPartionsForFailedRanges)));
            }
        } finally {
    ...
    private static String getTargetDC(Metadata metadata, String[] inputInitialAddress) {
        ...
        // only way to match is off the broadcast addresses, so for all hosts do a
        // existence check
        ...
        switch (dcToAddresses.keySet().size()) {
            ...
            case 0:
                throw new IllegalStateException(
                        "Input addresses could not be used to find DC; non match client metadata");
            default:
                // Mutliple DCs found, attempt to pick the first based off address list. This is
                // to mimic the 2.1
                // behavior which would connect in order and the first node successfully able to
                // connect to was the
                // local DC to use; since client abstracts this, we rely on existence as a proxy
                // for connect.
                for (String inputAddress : inputInitialAddress) {
                    for (InetAddress add : parseAddress(inputAddress)) {
                        String dc = addressToDc.get(add);
                        // possible the address isn't in the cluster and the client dropped, so ignore
                        // null
                        if (dc != null)
                            return dc;
                    }
                }
                // some how we were able to connect to the cluster, find multiple DCs using
                // matching, and yet couldn't
                // match again...
                throw new AssertionError(
                        "Unable to infer datacenter from initial addresses; multiple datacenters found "
    ...
    private Map<TokenRange, Long> getSubSplits(String keyspace, String cfName, TokenRange range, Host host,
            Configuration conf, Session session) {
    ...
    private Map<TokenRange, Long> describeSplits(String keyspace, String table, TokenRange tokenRange, Host host,
            int splitSize, int splitSizeMb, Session session) {
        // In 2.1 the host list was walked in-order (only move to next if IOException)
        // and calls
        // org.apache.cassandra.service.StorageService.getSplits(java.lang.String,
        // java.lang.String,
        // org.apache.cassandra.dht.Range<org.apache.cassandra.dht.Token>, int)
        // that call computes totalRowCountEstimate (used to compute #splits) then
        // splits the ring based off those estimates
        //
        // The main difference is that the estimates in 2.1 were computed based off the
        // data, so replicas could answer the estimates
        // In 3.0 we rely on the below CQL query which is local and only computes
        // estimates for the primary range; this
        // puts us in a sticky spot to answer, if the node fails what do we do? 3.0
        // behavior only matches 2.1 IFF all
    ...
    private static ResultSet queryTableEstimates(Session session, Host host, String keyspace, String table,
        ...
        try {
            ...
            Statement stmt = new SimpleStatement(query, keyspace, table, tokenRange.getStart().toString(),
                    tokenRange.getEnd().toString()).setHost(host);
            return session.execute(stmt);
        } catch (InvalidQueryException e) {
            // if the table doesn't exist, fall back to old table. This is likely to return
            // no records in a multi
