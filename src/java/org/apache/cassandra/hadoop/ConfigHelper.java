...
public class ConfigHelper {
    ...
    private static final String OUTPUT_COLUMNFAMILY_CONFIG = "mapreduce.output.basename"; // this must ==
                                                                                          // OutputFormat.BASE_OUTPUT_NAME
    ...
    /**
     * Set the size of the input split. setInputSplitSize value is used if this is
     * not set.
     * This affects the number of maps created, if the number is too small
     * the overhead of each map will take up the bulk of the job time.
     *
    ...
    public static String getOutputColumnFamily(Configuration conf) {
        if (conf.get(OUTPUT_COLUMNFAMILY_CONFIG) != null)
            ...
            throw new UnsupportedOperationException(
                    "You must set the output column family using either setOutputColumnFamily or by adding a named output with MultipleOutputs");
    }
