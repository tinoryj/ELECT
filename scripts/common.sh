...
function loadDataForEvaluation {
    ...
    for nodeIP in "${NodesList[@]}"; do
        ...
        scp ${UserName}@${OSSServerNode}:${PathToELECTLog}/${ExpName}-${targetScheme}-KVNumber-${record_count}-KeySize-${key_length}-ValueSize-${field_length}-CodingK-${codingK}-Saving-${storageSavingTarget}-OSSStorage.log ${PathToELECTResultSummary}/${targetScheme}/${ExpName}-${targetScheme}-KVNumber-${record_count}-KeySize-${key_length}-ValueSize-${field_length}-CodingK-${codingK}-Saving-${storageSavingTarget}-OSSStorage.log
    done
}
