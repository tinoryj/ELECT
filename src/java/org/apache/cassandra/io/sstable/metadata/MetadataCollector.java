...
public class MetadataCollector implements PartitionStatisticsCollector {
    ...
    // TODO: cound the number of row per partition (either with the number of cells,
    // or instead)
    ...
    protected StreamingTombstoneHistogramBuilder estimatedTombstoneDropTime = new StreamingTombstoneHistogramBuilder(
    ...
    public MetadataCollector updateClusteringValues(ClusteringPrefix<?> clustering) {
        minClustering = minClustering == null || comparator.compare(clustering, minClustering) < 0
                ? clustering.minimize()
                : minClustering;
        maxClustering = maxClustering == null || comparator.compare(clustering, maxClustering) > 0
                ? clustering.minimize()
                : maxClustering;
        return this;
    }
    ...
    private static List<ByteBuffer> makeList(ByteBuffer[] values) {
        // In most case, l will be the same size than values, but it's possible for it
        // to be smaller
