...
function load {
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        ...
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-Load-${round}-Node$i
...
function flush {
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        echo "Copy loading stats of ${targetScheme} back, node$i"
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-Compaction-${round}-Node$i
        ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
    done
}
...
function backup {
    ...
    sed -i "s/DATAPATH/${ExpName}-${round}/g" playbook-backup.yaml
...
for scheme in "${schemes[@]}"; do
    ...
    for ((round = 1; round <= roundNumber; round++)); do
        ...
        if [ "${waitFlush}" == "true" ]; then
            flush "${scheme}" "${waitTimeCeil}" "${round}"
        fi
