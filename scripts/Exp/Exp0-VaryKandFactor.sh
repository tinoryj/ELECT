...
function load {
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        ...
        scp -r elect@node$i:/home/elect/Results ${PathToELECTLog}/${targetScheme}/${ExpName}-TSS-${storageSaving}-K-${codingK}-Load-Node$i
...
function flush {
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        echo "Copy loading stats of ${targetScheme} back, node$i"
        scp -r elect@node$i:/home/elect/Results ${PathToELECTLog}/${targetScheme}/${ExpName}-TSS-${storageSaving}-K-${codingK}-Compact-Node$i
        ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
    done
}
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        for ((i = 1; i <= NodeNumber; i++)); do
            ...
            scp -r elect@node$i:/home/elect/Results ${PathToELECTLog}/"${targetScheme}"/"${ExpName}-TSS-${storageSaving}-K-${codingK}-${workload}-${round}-Node$i"
            ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
        done
    done
}
