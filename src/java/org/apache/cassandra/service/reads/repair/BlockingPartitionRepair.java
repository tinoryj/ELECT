...
public class BlockingPartitionRepair
    ...
    public BlockingPartitionRepair(DecoratedKey key, Map<Replica, Mutation> repairs, ReplicaPlan.ForWrite writePlan,
            Predicate<InetAddressAndPort> shouldBlockOn) {
        this.key = key;
        ...
        // there are some cases where logically identical data can return different
        // digests
        // For read repair, this would result in ReadRepairHandler being called with a
        // map of
        // empty mutations. If we'd also speculated on either of the read stages, the
        // number
        // of empty mutations would be greater than blockFor, causing the latch ctor to
        // throw
        // an illegal argument exception due to a negative start value. So here we clamp
        // it 0
    ...
    private PartitionUpdate mergeUnackedUpdates() {
        ...
        List<PartitionUpdate> updates = Lists
                .newArrayList(Iterables.transform(pendingRepairs.values(), BlockingPartitionRepair::extractUpdate));
    ...
    public void sendInitialRepairs() {
        ...
        for (Map.Entry<Replica, Mutation> entry : pendingRepairs.entrySet()) {
            ...
            Preconditions.checkArgument(destination.isFull(), "Can't send repairs to transient replicas: %s",
                    destination);
     ...
     * If the {@param timeoutAt} is a past time, the method returns immediately with
     * the repair result.
     * 
    ...
    /**
     * If it looks like we might not receive acks for all the repair mutations we
     * sent out, combine all
     * the unacked mutations and send them to the minority of nodes not involved in
     * the read repair data
     * read / write cycle. We will accept acks from them in lieu of acks from the
     * initial mutations sent
     * out, so long as we receive the same number of acks as repair mutations
     * transmitted. This prevents
     * misbehaving nodes from killing a quorum read, while continuing to guarantee
    ...
    public void maybeSendAdditionalWrites(long timeout, TimeUnit timeoutUnit) {
        ...
        for (Replica replica : newCandidates) {
            ...
            if (mutation == null) {
                mutation = BlockingReadRepairs.createRepairMutation(update, writePlan.consistencyLevel(),
                        replica.endpoint(), true);
