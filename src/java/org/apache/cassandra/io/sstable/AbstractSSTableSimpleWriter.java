...
abstract class AbstractSSTableSimpleWriter implements Closeable {
    ...
    protected static final AtomicReference<SSTableId> id = new AtomicReference<>(
    ...
    private static Descriptor createDescriptor(File directory, final String keyspace, final String columnFamily,
            final SSTableFormat.Type fmt) throws IOException {
    ...
    private static SSTableId getNextId(File directory, final String columnFamily) throws IOException {
        while (true) {
            try (Stream<Path> existingPaths = Files.list(directory.toPath())) {
                ...
                SSTableId newId = SSTableIdFactory.instance.defaultBuilder()
                        .generator(Stream.concat(existingIds, Stream.of(lastId))).get();
    ...
    /**
     * Returns a PartitionUpdate suitable to write on this writer for the provided
     * key.
     *
