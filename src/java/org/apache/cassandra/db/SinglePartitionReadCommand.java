...
public class SinglePartitionReadCommand extends ReadCommand implements SinglePartitionReadQuery {
     ...
     * @param nowInSec              the time in seconds to use are "now" for this
     *                              query.
     ...
     *
     * @return a newly created read command. The returned command will use no row
     ...
     *         query every columns for the table (without limit or row filtering)
     *         and be in forward order.
     */
     ...
     *         {@code key}. The returned query will
     *         query every columns (without limit or row filtering) and be in
     *         forward order.
     */
    public static SinglePartitionReadCommand create(TableMetadata metadata, int nowInSec, DecoratedKey key,
    ...
    @SuppressWarnings("resource") // we close the created iterator through closing the result of this method (and
                                  // SingletonUnfilteredPartitionIterator ctor cannot fail)
    protected UnfilteredPartitionIterator queryStorage(final ColumnFamilyStore cfs,
        ...
        // data is being tracked. This is only requested after an initial digest
        // mismatch
     ...
     * <p>
     * If the partition is cached, and the filter given is within its bounds, we
     * return
     * from cache, otherwise from disk.
     * <p>
     * If the partition is is not cached, we figure out what filter is "biggest",
     * read
    ...
    private UnfilteredRowIterator getThroughCache(ColumnFamilyStore cfs, ReadExecutionController executionController) {
        ...
        // To be able to cache what we read, what we read must at least covers what the
        // cache holds, that
        // is the 'rowsToCache' first rows of the partition. We could read those
        // 'rowsToCache' first rows
        // systematically, but we'd have to "extend" that to whatever is needed for the
        // user query that the
        // 'rowsToCache' first rows don't cover and it's not trivial with our existing
        // filters. So currently
        // we settle for caching what we read only if the user query does query the head
        // of the partition since
        // that's the common case of when we'll be able to use the cache anyway. One
        // exception is if we cache
        ...
        if (cacheFullPartitions || clusteringIndexFilter().isHeadFilter()) {
            ...
            try {
                ...
                UnfilteredRowIterator iter = fullPartitionRead(metadata(), nowInSec(), partitionKey())
                        .queryMemtableAndDisk(cfs, executionController);
                try {
                    // Use a custom iterator instead of DataLimits to avoid stopping the original
                    // iterator
                    ...
                    if (sentinelSuccess && !toCache.isEmpty()) {
                        ...
                        // Whether or not the previous replace has worked, our sentinel is not in the
                        // cache anymore
                        sentinelReplaced = true;
                    }

                    // We then re-filter out what this query wants.
                    // Note that in the case where we don't cache full partitions, it's possible
                    // that the current query is interested in more
     ...
     * 2) does not apply the query limit, nor the row filter (and so ignore 2ndary
     * indexes).
     ...
     * It is publicly exposed because there is a few places where that is exactly
     * what we want,
     * but it should be used only where you know you don't need thoses things.
     * <p>
     * Also note that one must have created a {@code ReadExecutionController} on the
     * queried table and we require it as
     * a parameter to enforce that fact, even though it's not explicitlly used by
     * the method.
     */
    ...
    private UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs,
         ...
         * 1) We query memtables and sstables simulateneously. This is our most generic
         * strategy and the one we use
         * unless we have a names filter that we know we can optimize futher.
         * 2) If we have a name filter (so we query specific rows), we can make a bet:
         * that all column for all queried row
         * will have data in the most recent sstable(s), thus saving us from reading
         * older ones. This does imply we
         * have a way to guarantee we have all the data for what is queried, which is
         * only possible for name queries
         * and if we have neither non-frozen collections/UDTs nor counters.
         * If a non-frozen collection or UDT is queried we can't guarantee that an older
         * sstable won't have some
         * elements that weren't in the most recent sstables.
         * Counters are intrinsically a collection of shards and so have the same
         * problem.
         * Counter tables are also special in the sense that their rows do not have
         * primary key liveness
         * as INSERT statements are not supported on counter tables. Due to that even if
         * only the primary key
         * columns where queried, querying SSTables in timestamp order will always be
         * less efficient for counter tables.
         * Also, if tracking repaired data then we skip this optimization so we can
         * collate the repaired sstables
        ...
        try {
            ...
            for (Memtable memtable : view.memtables) {
                ...
                UnfilteredRowIterator iter = memtable.rowIterator(partitionKey(), filter.getSlices(metadata()),
                ...
                inputCollector
                        .addMemtableIterator(RTBoundValidator.validate(iter, RTBoundValidator.Stage.MEMTABLE, false));
             ...
             * read like
             * in collectTimeOrderedData, but we still want to eliminate sstable whose
             * maxTimestamp < mostRecentTombstone
             ...
             * In other words, iterating in descending maxTimestamp order allow to do our
             * mostRecentPartitionTombstone
             * elimination in one pass, and minimize the number of sstables for which we
            ...
            for (SSTableReader sstable : view.sstables) {
                ...
                // as other replicas may not have seen this partition delete and so could
                // include
                ...
                if (shouldInclude(sstable)) {
                    ...
                    // 'iter' is added to iterators which is closed on exception, or through the
                    ...
                    // sstable contains no tombstone if maxLocalDeletionTime == Integer.MAX_VALUE,
                    // so we can safely skip those entirely
                    if (sstable.mayHaveTombstones()) {
                        // 'iter' is added to iterators which is closed on exception, or through the
                        // closing of the final merged iterator
                        @SuppressWarnings("resource")
                        UnfilteredRowIteratorWithLowerBound iter = makeIterator(cfs, sstable, metricsCollector);
                        // if the sstable contains a partition delete, then we must include it
                        // regardless of whether it
                        // shadows any other data seen locally as we can't guarantee that other replicas
                        // have seen it
            ...
            List<UnfilteredRowIterator> iterators = inputCollector.finalizeIterators(cfs, nowInSec(),
    ...
    private boolean shouldInclude(SSTableReader sstable) {
        // If some static columns are queried, we should always include the sstable: the
        // clustering values stats of the sstable
    ...
    /**
     * Return a wrapped iterator that when closed will update the sstables iterated
     * and READ sample metrics.
     * Note that we cannot use the Transformations framework because they greedily
     * get the static row, which
     * would cause all iterators to be initialized and hence all sstables to be
     * accessed.
     */
    @SuppressWarnings("resource")
    ...
    /**
     * Do a read by querying the memtable(s) first, and then each relevant sstables
     * sequentially by order of the sstable
     * max timestamp.
     *
     * This is used for names query in the hope of only having to query the 1 or 2
     * most recent query and then knowing nothing
     * more recent could be in the older sstables (which we can only guarantee if we
     * know exactly which row we queries, and if
     * no collection or counters are included).
     * This method assumes the filter is a {@code ClusteringIndexNamesFilter}.
     */
    private UnfilteredRowIterator queryMemtableAndSSTablesInTimestampOrder(ColumnFamilyStore cfs,
            ClusteringIndexNamesFilter filter, ReadExecutionController controller) {
        Tracing.trace("Acquiring sstable references");
        ColumnFamilyStore.ViewFragment view = cfs.select(View.select(SSTableSet.LIVE, partitionKey()));
        ...
        for (SSTableReader sstable : view.sstables) {
            ...
            if (!shouldInclude(sstable)) {
                // This mean that nothing queried by the filter can be in the sstable. One
                // exception is the top-level partition deletion
                ...
                // We need to get the partition deletion and include it if it's live. In any
                // case though, we're done with that sstable.
    ...
    private ImmutableBTreePartition add(UnfilteredRowIterator iter, ImmutableBTreePartition result,
            ClusteringIndexNamesFilter filter, boolean isRepaired, ReadExecutionController controller) {
        if (!isRepaired)
    ...
    private ClusteringIndexNamesFilter reduceFilter(ClusteringIndexNamesFilter filter, ImmutableBTreePartition result,
            long sstableTimestamp) {
        if (result == null)
            return filter;

        // According to the CQL semantics a row exists if at least one of its columns is
        // not null (including the primary key columns).
        // Having the queried columns not null is unfortunately not enough to prove that
        // a row exists as some column deletion
        // for the queried columns can exist on another node.
        // For CQL tables it is enough to have the primary key liveness and the queried
        // columns as the primary key liveness prove that
        // the row exists even if all the other columns are deleted.
        // COMPACT tables do not have primary key liveness and by consequence we are
        // forced to get all the fetched columns to ensure that
        // we can return the correct result if the queried columns are deleted on
        // another node but one of the non-queried columns is not.
        RegularAndStaticColumns columns = metadata().isCompactTable() ? columnFilter().fetchedColumns()
                : columnFilter().queriedColumns();

        NavigableSet<Clustering<?>> clusterings = filter.requestedRows();

        // We want to remove rows for which we have values for all requested columns. We
        // have to deal with both static and regular rows.
    ...
    /**
     * We can stop reading row data from disk if what we've already read is more
     * recent than the max timestamp
     * of the next newest SSTable that might have data for the query. We care about
     * 1.) the row timestamp (since
     ...
     *
     * @return true if the supplied {@link Row} is complete and its data more recent
    ...
    private boolean isRowComplete(Row row, Columns requestedColumns, long sstableTimestamp) {
        ...
        if (!row.isStatic()) {
            // If the row has been deleted or is part of a range deletion we know that we
            // have enough information and can
            // stop at this point.
            // Note that deleted rows in compact tables (non static) do not have a row
            // deletion. Single column
            // cells are deleted instead. By consequence this check will not work for those,
            // but the row will appear as complete later on
            // in the method.
            ...
            if (!metadata().isCompactTable() && (row.primaryKeyLivenessInfo().isEmpty()
    ...
    public static class Group extends SinglePartitionReadQuery.Group<SinglePartitionReadCommand> {
        ...
        public static Group create(List<SinglePartitionReadCommand> commands, DataLimits limits) {
            return commands.get(0).metadata().isVirtual() ? new VirtualTableGroup(commands, limits)
    ...
    private static class Deserializer extends SelectionDeserializer {
        public ReadCommand deserialize(DataInputPlus in,
            ...
            return SinglePartitionReadCommand.create(isDigest, digestVersion, acceptsTransient, metadata, nowInSec,
                    columnFilter, rowFilter, limits, key, filter, index, false);
        }
    }

    /**
     * {@code SSTableReaderListener} used to collect metrics about SSTable read
     * access.
     */
    private static final class SSTableReadMetricsCollector implements SSTableReadsListener {
        /**
         * The number of SSTables that need to be merged. This counter is only updated
    ...
    public static class VirtualTableSinglePartitionReadCommand extends SinglePartitionReadCommand {
        protected VirtualTableSinglePartitionReadCommand(boolean isDigest,
            ...
            super(isDigest, digestVersion, acceptsTransient, metadata, nowInSec, columnFilter, rowFilter, limits,
                    partitionKey, clusteringIndexFilter, index, trackWarnings);
        }
        ...
        public UnfilteredPartitionIterator executeLocally(ReadExecutionController executionController) {
            ...
            UnfilteredPartitionIterator resultIterator = view.select(partitionKey, clusteringIndexFilter,
                    columnFilter());
