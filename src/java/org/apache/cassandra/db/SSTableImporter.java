...
public class SSTableImporter {
     ...
     *
     * If import fails in any of the directories, that directory is skipped and the
     * failed directories
     * are returned so that the user can re-upload files or remove corrupt files.
     *
     * If one of the directories in srcPaths is not readable/does not exist, we exit
     * immediately to let
    ...
    synchronized List<String> importNewSSTables(Options options) {
        ...
        // verify first to avoid starting to copy sstables to the data directories and
        // then have to abort.
            ...
            for (Pair<Directories.SSTableLister, String> listerPair : listers) {
                ...
                for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet()) {
                    ...
                    if (!currentDescriptors.contains(entry.getKey())) {
                        try {
                            verifySSTableForImport(descriptor, entry.getValue(), options.verifyTokens,
                                    options.verifySSTables, options.extendedVerify);
                        } catch (Throwable t) {
                            if (dir != null) {
        ...
        for (Pair<Directories.SSTableLister, String> listerPair : listers) {
            ...
            for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet()) {
                try {
                    ...
                    SSTableReader sstable = SSTableReader.moveAndOpenSSTable(cfs, entry.getKey(), newDescriptor,
                    ...
                    if (dir != null) {
                        ...
                        logger.error("Failed importing sstables from data directory - renamed sstables are: {}",
                                movedSSTables);
        ...
        logger.info("Loading new SSTables and building secondary indexes for {}/{}: {}", cfs.keyspace.getName(),
                cfs.getTableName(), newSSTables);
    ...
    /**
     * Opens the sstablereader described by descriptor and figures out the correct
     * directory for it based
     * on the first token
     *
     * srcPath == null means that the sstable is in a data directory and we can use
     * that directly.
     *
     * If we fail figuring out the directory we will pick the one with the most
    ...
    private File getTargetDirectory(String srcPath, Descriptor descriptor, Set<Component> components) {
        ...
        try {
            ...
            targetDirectory = cfs.getDirectories().getLocationForDisk(
                    cfs.diskBoundaryManager.getDiskBoundaries(cfs).getCorrectDiskForSSTable(sstable));
        } finally {
            if (sstable != null)
                sstable.selfRef().release();
        }
        return targetDirectory == null
                ? cfs.getDirectories().getWriteableLocationToLoadFile(new File(descriptor.baseFilename()))
                : targetDirectory;
    }
     ...
     *
     * If srcPaths is empty, we create a lister that lists sstables in the data
     * directories (deprecated use)
     */
    private List<Pair<Directories.SSTableLister, String>> getSSTableListers(Set<String> srcPaths) {
        ...
        if (!srcPaths.isEmpty()) {
            for (String path : srcPaths) {
                ...
                listers.add(Pair.create(
                        cfs.getDirectories().sstableLister(dir, Directories.OnTxnErr.IGNORE).skipTemporary(true),
                        path));
            }
        } else {
            listers.add(Pair.create(cfs.getDirectories().sstableLister(Directories.OnTxnErr.IGNORE).skipTemporary(true),
                    null));
        }
    ...
    /**
     * If we fail when opening the sstable (if for example the user passes in
     * --no-verify and there are corrupt sstables)
     * we might have started copying sstables to the data directory, these need to
     * be moved back to the original name/directory
     */
    private void moveSSTablesBack(Set<MovedSSTable> movedSSTables) {
        for (MovedSSTable movedSSTable : movedSSTables) {
            if (new File(movedSSTable.newDescriptor.filenameFor(Component.DATA)).exists()) {
                ...
                logger.debug("Moving sstable {} back to {}",
    ...
    /**
     * Similarly for moving case, we need to delete all SSTables which were copied
     * already but the
     * copying as a whole has failed so we do not leave any traces behind such
     * failed import.
     *
     * @param movedSSTables tables we have moved already (by copying) which need to
     *                      be removed
     */
    ...
    /**
     * Verify an sstable for import, throws exception if there is a failure
     * verifying.
     *
     * @param verifyTokens   to verify that the tokens are owned by the current node
     * @param verifySSTables to verify the sstables given. If this is false a
     *                       "quick" verification will be run, just deserializing
     *                       metadata
     * @param extendedVerify to validate the values in the sstables
     */
    private void verifySSTableForImport(Descriptor descriptor, Set<Component> components, boolean verifyTokens,
            boolean verifySSTables, boolean extendedVerify) {
    ...
    /**
     * Depending on the options passed in, this might reset level on the sstable to
     * 0 and/or remove the repair information
     * from the sstable
     */
    ...
    public static class Options {
        ...
        public Options(Set<String> srcPaths, boolean resetLevel, boolean clearRepaired, boolean verifySSTables,
        ...
        static class Builder {
            ...
            public Options build() {
                return new Options(srcPaths, resetLevel, clearRepaired, verifySSTables, verifyTokens, invalidateCaches,
