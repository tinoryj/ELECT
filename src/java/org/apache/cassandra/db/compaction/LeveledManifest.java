...
public class LeveledManifest {
    ...
    /**
     * if we have more than MAX_COMPACTING_L0 sstables in L0, we will run a round of
     * STCS with at most
    ...
    LeveledManifest(ColumnFamilyStore cfs, int maxSSTableSizeInMB, int fanoutSize,
    ...
    public static LeveledManifest create(ColumnFamilyStore cfs, int maxSSTableSize, int fanoutSize,
            List<SSTableReader> sstables) {
        return create(cfs, maxSSTableSize, fanoutSize, sstables, new SizeTieredCompactionStrategyOptions());
    }

    public static LeveledManifest create(ColumnFamilyStore cfs, int maxSSTableSize, int fanoutSize,
            Iterable<SSTableReader> sstables, SizeTieredCompactionStrategyOptions options) {
    ...
    /**
     * If we want to start compaction in level n, find the newest (by modification
     * time) file in level n+1
    ...
    public synchronized CompactionCandidate getCompactionCandidates() {
        ...
        // L2: 12 [ideal: 100]
        //
        // The problem is that L0 has a much higher score (almost 250) than L1 (11), so
        // what we'll
        // do is compact a batch of cfs.getMaximumCompactionThreshold() sstables with
        // all 117 L1 sstables, and put the
        // result (say, 120 sstables) in L1. Then we'll compact the next batch of
        // cfs.getMaxCompactionThreshold(),
        // and so forth. So we spend most of our i/o rewriting the L1 data with each
        // batch.
        //
        // If we could just do *all* L0 a single time with L1, that would be ideal. But
        // we can't
        ...
        // 2) if L0 falls behind, we will size-tiered compact it to reduce read overhead
        // until
        // we can catch up on the higher levels.
        //
        // This isn't a magic wand -- if you are consistently writing too fast for LCS
        // to keep
        // up, you're still screwed. But if instead you have intermittent bursts of
        // activity,
        // it can help a lot.
        ...
        for (int i = generations.levelCount() - 1; i > 0; i--) {
            ...
            if (score > 1.001) {
                // the highest level should not ever exceed its maximum size under normal
                // curcumstaces,
        ...
        if (candidates.isEmpty()) {
            ...
            // there is a long running compaction, we want to make sure that we continue to
            // keep the number of SSTables
            // small in L0.
            return l0Compaction;
        }
    ...
    private List<SSTableReader> getSSTablesForSTCS(Collection<SSTableReader> sstables) {
        ...
        List<Pair<SSTableReader, Long>> pairs = SizeTieredCompactionStrategy
                .createSSTableAndLengthPairs(AbstractCompactionStrategy.filterSuspectSSTables(candidates));
     ...
     * change sstable size) we will "never"
     * compact the high levels.
     *
     * This method finds if we have gone many compaction rounds without doing any
     * high-level compaction, if so
     * we start bringing in one sstable from the highest level until that level is
     * either empty or is doing compaction.
     *
     * @param targetLevel the level the candidates will be compacted into
     * @param candidates  the original sstables to compact
     * @return
     */
    private Collection<SSTableReader> getOverlappingStarvedSSTables(int targetLevel,
        ...
        for (int i = generations.levelCount() - 1; i > 0; i--) {
            if (getLevelSize(i) > 0) {
                if (compactionCounter[i] > NO_COMPACTION_LIMIT) {
                    // we try to find an sstable that is fully contained within the boundaries we
                    // are compacting;
                    ...
                    if (min == null || max == null || min.equals(max)) // single partition sstables - we cannot include
                                                                       // a high level sstable.
                        return candidates;
    ...
    private static Set<SSTableReader> overlapping(Collection<SSTableReader> candidates,
            Iterable<SSTableReader> others) {
        assert !candidates.isEmpty();
        /*
         * Picking each sstable from others that overlap one of the sstable of
         * candidates is not enough
         * because you could have the following situation:
         * candidates = [ s1(a, c), s2(m, z) ]
         * others = [ s3(e, g) ]
         * In that case, s2 overlaps none of s1 or s2, but if we compact s1 with s2, the
         * resulting sstable will
         * overlap s3, so we must return s3.
         *
         * Thus, the correct approach is to pick sstables overlapping anything between
         * the first key in all
    ...
    /**
     * @return sstables from @param sstables that contain keys between @param start
    ...
    private static boolean isSelectIssuedSSTableAsCompactionCandidates(SSTableReader sstable) {
        ...
        long delayForFirstTimeParityUpdate = DatabaseDescriptor.getTaskDelay() * 60 * 1000;
    ...
    public synchronized int getEstimatedTasks() {
        ...
        for (int i = generations.levelCount() - 1; i >= 0; i--) {
            ...
            estimated[i] = (long) Math.ceil((double) Math.max(0L,
                    SSTableReader.getTotalBytes(sstables) - (long) (maxBytesForLevel(i, maxSSTableSizeInBytes) * 1.001))
    ...
    public int getNextLevel(Collection<SSTableReader> sstables) {
        ...
        int newLevel;
        if (minimumLevel == 0 && minimumLevel == maximumLevel
                && SSTableReader.getTotalBytes(sstables) < maxSSTableSizeInBytes) {
            newLevel = 0;
        } else {
    ...
    public int getHighestLevel(Collection<SSTableReader> sstables) {
        int maximumLevel = Integer.MIN_VALUE;
        int minimumLevel = Integer.MAX_VALUE;
        for (SSTableReader sstable : sstables) {
            maximumLevel = Math.max(sstable.getSSTableLevel(), maximumLevel);
