...
targetStorageSavings=("0.4" "0.6" "0.8")
codingK=6
...
function load {
    ...
    if [ ${targetScheme} == "elect" ]; then
        ...
        sed -i "s/\(target_saving: \)".*"/target_saving: ${storageSaving}/" playbook-load.yaml
        sed -i "s/\(data_block_num: \)".*"/data_block_num: ${codingK}/" playbook-load.yaml
    else
    ...
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${targetScheme}-TSS-${storageSaving}-K-${codingK}-Load"/" playbook-load.yaml
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        ...
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-TSS-${storageSaving}-K-${codingK}-Load-Node$i
...
function flush {
    ...
    # Modify playbook
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${storageSaving}-${targetScheme}-K-${codingK}-Load"/" playbook-flush.yaml
    ...
    for ((i = 1; i <= NodeNumber; i++)); do
        echo "Copy loading stats of ${targetScheme} back, node$i"
        scp -r elect@node$i:/home/elect/Results /home/elect/Results/${targetScheme}/${ExpName}-TSS-${storageSaving}-K-${codingK}-Compact-Node$i
        ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
    done
}
...
function backup {
    ...
    sed -i "s/DATAPATH/${ExpName}-${storageSaving}-${codingK}/g" playbook-backup.yaml
...
function startup {
    ...
    if [ ${targetScheme} == "elect" ]; then
        ...
        sed -i "s/\(data_block_num: \)".*"/data_block_num: ${codingK}/" playbook-startup.yaml
    else
    ...
    sed -i "s/DATAPATH/${ExpName}-${storageSaving}-${codingK}/g" playbook-startup.yaml
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${targetScheme}-Run-${runningType}-TSS-${storageSaving}-K-${codingK}-Round-${round}"/" playbook-run.yaml
        ...
        for ((i = 1; i <= NodeNumber; i++)); do
            ...
            scp -r elect@node$i:/home/elect/Results /home/elect/Results/"${targetScheme}"/"${ExpName}-TSS-${storageSaving}-K-${codingK}-${workload}-${round}-Node$i"
            ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
        done
    done
}
...
for scheme in "${schemes[@]}"; do
    ...
    if [ "${scheme}" == "elect" ]; then
        waitTime=$(echo "scale=2; ($dataSizeOnEachNode * 1024 / 4 / 2 / ($concurrentEC / 2)) * 120 + $dataSizeOnEachNode * 450" | bc)
        maxLevelCount
    ...
    for storageSaving in "${targetStorageSavings[@]}"; do
        load "${scheme}" "${storageSaving}"

        if [ "${waitFlush}" == "true" ]; then
            flush "${scheme}" "${waitTimeCeil}" "${storageSaving}"
        fi

        if [ "${restart}" == "true" ]; then
            backup "${scheme}" "${storageSaving}"
        fi
        ...
        for ((round = 1; round <= roundNumber; round++)); do
            for type in "${types[@]}"; do
                if [ "${type}" == "normal" ]; then
                    if [ "${restart}" == "true" ]; then
                        startup "${scheme}" "${storageSaving}"
                    fi
                    run "${scheme}" "${round}" "${type}" "${storageSaving}"
