...
# Exp7: SYN workload, 3-way replication, (6,4) encoding, 60% target storage saving, 10M KV + 1M OP. Read Consistency level
...
function load {
    ...
    if [ ${targetScheme} == "elect" ]; then
        ...
        sed -i "s/\(target_saving: \)".*"/target_saving: 0.6/" playbook-load.yaml
    else
...
function flush {
    ...
    # Modify playbook
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-Load"/" playbook-flush.yaml
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        sed -i "s/\(expName: \)".*"/expName: ${ExpName}-${targetScheme}-Run-${runningType}-CL-${consistency}-Round-${round}/" playbook-run.yaml
        ...
        for ((i = 1; i <= NodeNumber; i++)); do
            ...
            scp -r elect@node$i:/home/elect/Results /home/elect/Results/"${targetScheme}"/"${ExpName}-CL-${consistency}-${workload}-${round}-Node$i"
            ssh elect@node$i "rm -rf /home/elect/Results && mkdir -p /home/elect/Results"
        done
    done
}
...
for scheme in "${schemes[@]}"; do
    ...
    for consistency in "${consistencyLevels[@]}"; do
        for ((round = 1; round <= roundNumber; round++)); do
            for type in "${types[@]}"; do
                if [ "${type}" == "normal" ]; then
                    ...
                    run "${scheme}" "${round}" "${type}" "${consistency}"
