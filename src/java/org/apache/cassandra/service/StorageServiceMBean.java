...
public interface StorageServiceMBean extends NotificationEmitter {
    ...
    /**
     * @deprecated use
     *             {@link #takeSnapshot(String tag, Map options, String... entities)}
     *             instead.
     */
    @Deprecated
    ...
    @Deprecated
    public int relocateSSTables(String keyspace, String... cfnames)
            throws IOException, ExecutionException, InterruptedException;
    ...
    /**
     * Scrub (deserialize + reserialize at the latest version, skipping bad rows if
     * any) the given keyspace.
    ...
    @Deprecated
    default int upgradeSSTables(String keyspaceName, boolean excludeCurrentVersion, int jobs, String... tableNames)
            throws IOException, ExecutionException, InterruptedException {
        return upgradeSSTables(keyspaceName, excludeCurrentVersion, Long.MAX_VALUE, jobs, tableNames);
    }

    public int upgradeSSTables(String keyspaceName, boolean excludeCurrentVersion, long maxSSTableTimestamp, int jobs,
            String... tableNames) throws IOException, ExecutionException, InterruptedException;
     ...
     * The tombstone option defines the granularity of the procedure: ROW removes
     * deleted partitions and rows, CELL also removes overwritten or deleted cells.
     */
    public int garbageCollect(String tombstoneOption, int jobs, String keyspaceName, String... tableNames)
            throws IOException, ExecutionException, InterruptedException;

    /**
     * Flush all memtables for the given column families, or all columnfamilies for
     * the given keyspace
    ...
    /**
     * Invoke recovery asynchronously.
     * You can track recovery progress by subscribing JMX notification sent from
     * this
     * StorageServiceMBean.
     * Notification format is:
     * type: recovery"
     ...
     * ActiveRepairService.Status
     *
     * @param keyspace Keyspace name to recovery. Should not be null.
     ...
     * @return status of parent repair from enum
     *         {@link org.apache.cassandra.repair.RepairRunnable.Status}
     ...
     *                 This node will unload its data onto its neighbors, and
     *                 bootstrap to the new token.
     */
     ...
     * to null for the defined classQualifer<br>
     ...
     * parameters.
     * If {@code epSnitchClassName} is specified, it will configure a new snitch
     * instance and make it a
     * 'dynamic snitch' if {@code dynamic} is specified and {@code true}.
     *
     * The parameters {@code dynamicUpdateInterval}, {@code dynamicResetInterval}
     * and {@code dynamicBadnessThreshold}
     * can be specified individually to update the parameters of the dynamic snitch
     * during runtime.
     *
     ...
     *                                used or not - only valid, if
     *                                {@code epSnitchClassName} is specified
     * @param dynamicUpdateInterval   integer, in ms (defaults to the value
     *                                configured in cassandra.yaml, which defaults
     *                                to 100)
     * @param dynamicResetInterval    integer, in ms (defaults to the value
     *                                configured in cassandra.yaml, which defaults
     *                                to 600,000)
     ...
     * Note this becomes false before
     * {@link org.apache.cassandra.db.SystemKeyspace#bootstrapComplete()} is called,
     * as setting bootstrap to complete is called only when the node joins the ring.
     * 
     * @return True prior to bootstrap streaming completing. False prior to start of
     *         bootstrap and post streaming.
     */
     ...
     * @deprecated Use getStreamThroughputMbitPerSecAsDouble instead as this one
     *             will provide a rounded value
     */
    @Deprecated
     ...
     * @deprecated Use getStreamThroughputMebibytesPerSecAsDouble instead as this
    ...
    /**
     * Below method returns stream_throughput_outbound rounded, for precise number,
     * please, use getStreamThroughputMebibytesPerSecAsDouble
     * 
     * @return stream_throughput_outbound in MiB
     */
     ...
     * @deprecated Use getInterDCStreamThroughputMbitPerSecAsDouble instead as this
     *             one will provide a rounded value
     */
    @Deprecated
     ...
     * @deprecated Use getInterDCStreamThroughputMebibytesPerSecAsDouble instead as
     *             this one will provide a rounded value
     */
    @Deprecated
    ...
    /**
     * Below method returns Inter_dc_stream_throughput_outbound rounded, for precise
     * number, please, use getInterDCStreamThroughputMebibytesPerSecAsDouble
     * 
     * @return inter_dc_stream_throughput_outbound in MiB
     */
     ...
     * @deprecated Use getCompactionThroughtputMibPerSecAsDouble instead as this one
     *             will provide a rounded value
     */
    @Deprecated
    ...
    /**
     * Initiate a process of streaming data for which we are responsible from other
     * nodes. It is similar to bootstrap
     * except meant to be used on a node which is already in the cluster (typically
     * containing no data) as an
     * alternative to running repair.
     *
     * @param sourceDc Name of DC from which to select sources for streaming or null
     *                 to pick any node
     */
     ...
     * @param sourceDc Name of DC from which to select sources for streaming or null
     *                 to pick any node
     * @param keyspace Name of the keyspace which to rebuild or null to rebuild all
     *                 keyspaces.
    ...
    /**
     * Starts a bulk load asynchronously and returns the String representation of
     * the planID for the new
     * streaming session.
     */
     ...
     * ColumnFamilyStores.
     *
     * Note: this should be left as an operation, not an attribute (methods starting
     * with "get")
     * to avoid sending potentially multiple MB of data when accessing this mbean by
     * default. See CASSANDRA-4452.
     *
     ...
     *                    disable tracing and 1 will enable tracing for all requests
     *                    (which mich severely cripple the system)
     */
    public void setTraceProbability(double probability);

    public Map<String, List<CompositeData>> samplePartitions(int duration, int capacity, int count,
    ...
    /**
     * Returns the number of rows cached at the coordinator before filtering/index
     * queries log a warning.
     */
    public int getCachedReplicaRowsWarnThreshold();

    /**
     * Sets the number of rows cached at the coordinator before filtering/index
     * queries log a warning.
     */
    ...
    /**
     * Returns the threshold for skipping the column index when caching partition
     * info
     **/
     ...
     *
     *
     * @return true if the node successfully starts resuming. (this does not mean
     *         bootstrap streaming was success.)
     */
     ...
     * 
     * @param path              Path where the full query log will be stored. If
     *                          null cassandra.yaml value is used.
     ...
     *                          should block if they can't be drained to the
     *                          filesystem or alternatively drops samples and log
     ...
     *                          segments. Might not be respected if a log file
     *                          hasn't rolled so it can be deleted.
     * @param archiveCommand    executable archiving the rolled log files,
     * @param maxArchiveRetries max number of times to retry a failing archive
     *                          command
     *
     */
    public void enableFullQueryLogger(String path, String rollCycle, Boolean blocking, int maxQueueWeight,
            long maxLogSize, @Nullable String archiveCommand, int maxArchiveRetries);

    /**
     * Disable the full query logger if it is enabled.
     * Also delete any generated files in the last used full query log path as well
     * as the one configure in cassandra.yaml
     */
    ...
    /**
     * Sets the initial allocation size of backing arrays for new RangeTombstoneList
     * objects
     */
    ...
    /**
     * Returns a map of schema version -> list of endpoints reporting that version
     * that we need schema updates for
     */
    @Deprecated
