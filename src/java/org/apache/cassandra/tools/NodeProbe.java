...
public class NodeProbe implements AutoCloseable {
    ...
    static long JMX_NOTIFICATION_POLL_INTERVAL_SECONDS = Long.getLong(
            "cassandra.nodetool.jmx_notification_poll_interval_seconds", TimeUnit.SECONDS.convert(5, TimeUnit.MINUTES));
    ...
    protected NodeProbe() {
        // this constructor is only used for extensions to rewrite their own connect
        // method
        this.host = "";
        this.port = 0;
        this.output = Output.CONSOLE;
    ...
    protected void connect() throws IOException {
        ...
        if (host.contains(":")) {
            // Use square brackets to surround IPv6 addresses to fix CASSANDRA-7669 and
            // CASSANDRA-17581
            host = "[" + host + "]";
        }
    ...
    public void close() throws IOException {
        try {
            ...
            // result of 'stopdaemon' command - i.e. if close() call fails, the daemon is
            // shutdown
    ...
    public int scrub(boolean disableSnapshot, boolean skipCorrupted, boolean checkData, boolean reinsertOverflowedTTL,
            ...
            throws IOException, ExecutionException, InterruptedException {
        return ssProxy.scrub(disableSnapshot, skipCorrupted, checkData, reinsertOverflowedTTL, jobs, keyspaceName,
                tables);
    }
    ...
    public int verify(boolean extendedVerify, boolean checkVersion, boolean diskFailurePolicy,
        ...
        return ssProxy.verify(extendedVerify, checkVersion, diskFailurePolicy, mutateRepairStatus, checkOwnsTokens,
                quick, keyspaceName, tableNames);
    }
    ...
    private void checkJobs(PrintStream out, int jobs) {
        ...
        if (jobs > compactors)
            out.println(String.format(
                    "jobs (%d) is bigger than configured concurrent_compactors (%d) on the host, using at most %d threads",
                    jobs, compactors, compactors));
    }

    public void forceKeyspaceCleanup(PrintStream out, int jobs, String keyspaceName, String... tableNames)
            throws IOException, ExecutionException, InterruptedException {
        checkJobs(out, jobs);
    ...
    public void scrub(PrintStream out, boolean disableSnapshot, boolean skipCorrupted, boolean checkData,
            boolean reinsertOverflowedTTL, int jobs, String keyspaceName, String... tables)
            throws IOException, ExecutionException, InterruptedException {
        checkJobs(out, jobs);
        perform(out, keyspaceName,
                () -> scrub(disableSnapshot, skipCorrupted, checkData, reinsertOverflowedTTL, jobs, keyspaceName,
                        tables),
                "scrubbing");
    }

    public void verify(PrintStream out, boolean extendedVerify, boolean checkVersion, boolean diskFailurePolicy,
            boolean mutateRepairStatus, boolean checkOwnsTokens, boolean quick, String keyspaceName,
            String... tableNames) throws IOException, ExecutionException, InterruptedException {
        perform(out, keyspaceName,
                () -> verify(extendedVerify, checkVersion, diskFailurePolicy, mutateRepairStatus, checkOwnsTokens,
                        quick, keyspaceName, tableNames),
                "verifying");
    }

    public void recompressSSTables(PrintStream out, String keyspaceName, int jobs, String... tableNames)
            throws IOException, ExecutionException, InterruptedException {
        checkJobs(out, jobs);
        perform(out, keyspaceName,
    ...
    public void upgradeSSTables(PrintStream out, String keyspaceName, boolean excludeCurrentVersion,
            long maxSSTableTimestamp, int jobs, String... tableNames)
            throws IOException, ExecutionException, InterruptedException {
        checkJobs(out, jobs);
        perform(out, keyspaceName,
    ...
    private void perform(PrintStream out, String ks, Job job, String jobName)
            throws IOException, ExecutionException, InterruptedException {
        switch (job.perform()) {
            case 1:
                out.printf(
                        "Aborted %s for at least one table in keyspace %s, check server logs for more information.\n",
                        jobName, ks);
                break;
            case 2:
                failed = true;
    ...
    public void garbageCollect(PrintStream out, String tombstoneOption, int jobs, String keyspaceName,
            String... tableNames) throws IOException, ExecutionException, InterruptedException {
        if (garbageCollect(tombstoneOption, jobs, keyspaceName, tableNames) != 0) {
            failed = true;
            out.println("Aborted garbage collection for at least one table in keyspace " + keyspaceName
     ...
     */
    public void forceKeyspaceCompactionForTokenRange(String keyspaceName, final String startToken,
            final String endToken, String... tableNames) throws IOException, ExecutionException, InterruptedException {
    ...
    public void reloadImMemoryDataForElectColdStartup() {
        ssProxy.reloadImMemoryDataForElectColdStartup();
    }
    ...
    public void recoveryAsync(final PrintStream out, final String keyspace, String cfsName)
            throws IOException {
    ...
    public Map<String, List<CompositeData>> getPartitionSample(int capacity, int durationMillis, int count,
            List<String> samplers) throws OpenDataException {
        return ssProxy.samplePartitions(durationMillis, capacity, count, samplers);
    }

    public Map<String, List<CompositeData>> getPartitionSample(String ks, String cf, int capacity, int durationMillis,
            int count, List<String> samplers) throws OpenDataException {
    ...
    /**
     * Take a snapshot of all the keyspaces, optionally specifying only a specific
     * column family.
     *
     ...
     * @param tableList
     *                     list of columnfamily from different keyspace in the form
     *                     of ks1.cf1 ks2.cf2
     */
    ...
    public EndpointSnitchInfoMBean getEndpointSnitchInfoProxy() {
        try {
            return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.db:type=EndpointSnitchInfo"),
    ...
    public DynamicEndpointSnitchMBean getDynamicEndpointSnitchInfoProxy() {
        try {
            return JMX.newMBeanProxy(mbeanServerConn,
                    new ObjectName("org.apache.cassandra.db:type=DynamicEndpointSnitch"),
                    DynamicEndpointSnitchMBean.class);
        } catch (MalformedObjectNameException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public ColumnFamilyStoreMBean getCfsProxy(String ks, String cf) {
        ...
        try {
            ...
            Set<ObjectName> beans = mbeanServerConn.queryNames(
                    new ObjectName("org.apache.cassandra.db:type=*" + type + ",keyspace=" + ks + ",columnfamily=" + cf),
                    null);
    ...
    public List<String> importNewSSTables(String ksName, String cfName, Set<String> srcPaths, boolean resetLevel,
            boolean clearRepaired, boolean verifySSTables, boolean verifyTokens, boolean invalidateCaches,
            boolean extendedVerify, boolean copyData) {
        return getCfsProxy(ksName, cfName).importNewSSTables(srcPaths, resetLevel, clearRepaired, verifySSTables,
    ...
    public String getGossipInfo(boolean withPort, boolean resolveIp) {
        if (resolveIp)
            return withPort ? fdProxy.getAllEndpointStatesWithPortAndResolveIp()
    ...
    public Object getCacheMetric(String cacheType, String metricName) {
        try {
            switch (metricName) {
                ...
                case "Size":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.cassandra.metrics:type=Cache,scope=" + cacheType + ",name="
                                    + metricName),
                ...
                case "MissLatencyUnit":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName(
                                    "org.apache.cassandra.metrics:type=Cache,scope=" + cacheType + ",name=MissLatency"),
     ...
     * 
     * @param ks         Keyspace for which stats are to be displayed or null for
     *                   the global value
     * @param cf         ColumnFamily for which stats are to be displayed or null
     *                   for the keyspace value (if ks supplied)
    ...
    public Object getColumnFamilyMetric(String ks, String cf, String metricName) {
        try {
            ...
            if (!Strings.isNullOrEmpty(ks) && !Strings.isNullOrEmpty(cf)) {
                ...
                oName = new ObjectName(String.format(
                        "org.apache.cassandra.metrics:type=%s,keyspace=%s,scope=%s,name=%s", type, ks, cf, metricName));
            } else if (!Strings.isNullOrEmpty(ks)) {
                oName = new ObjectName(String.format("org.apache.cassandra.metrics:type=Keyspace,keyspace=%s,name=%s",
                        ks, metricName));
            } else {
    ...
    public Object getClientMetric(String metricName) {
        try {
            switch (metricName) {
                case "connections": // List<Map<String,String>> - list of all native connections and their
                                    // properties
    ...
    public void setLoggingLevel(String classQualifier, String level) {
        try {
            ...
            throw new RuntimeException("Error setting log for " + classQualifier + " on level " + level
                    + ". Please check logback configuration and ensure to have <jmxConfigurator /> set", e);
        }
    }
    ...
    public void enableAuditLog(String loggerName, Map<String, String> parameters, String includedKeyspaces,
        ...
        ssProxy.enableAuditLog(loggerName, parameters, includedKeyspaces, excludedKeyspaces, includedCategories,
    ...
    public void enableAuditLog(String loggerName, String includedKeyspaces, String excludedKeyspaces,
        ...
        this.enableAuditLog(loggerName, Collections.emptyMap(), includedKeyspaces, excludedKeyspaces,
    ...
    public void enableFullQueryLogger(String path, String rollCycle, Boolean blocking, int maxQueueWeight,
        ...
        ssProxy.enableFullQueryLogger(path, rollCycle, blocking, maxQueueWeight, maxLogSize, archiveCommand,
                maxArchiveRetries);
    }
