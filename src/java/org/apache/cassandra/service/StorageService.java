...
public class StorageService extends NotificationBroadcasterSupport
    ...
    public volatile VersionedValue.VersionedValueFactory valueFactory = new VersionedValue.VersionedValueFactory(
    ...
    /*
     * the probability for tracing any particular request, 0 disables tracing and 1
     * enables for all
     */
    ...
    public static final boolean useStrictConsistency = Boolean
            .parseBoolean(System.getProperty("cassandra.consistent.rangemovement", "true"));
    private static final boolean allowSimultaneousMoves = Boolean
            .parseBoolean(System.getProperty("cassandra.consistent.simultaneousmoves.allow", "false"));
    ...
    public synchronized void startGossiping() {
        if (!gossipActive) {
            ...
            // shouldn't be called before these are set if we intend to join the ring/are in
    ...
    public void stopClient() {
        ...
        // give it a second so that task accepted before the MessagingService shutdown
        // gets submitted to the stage (to avoid RejectedExecutionException)
    ...
    private synchronized UUID prepareForReplacement() throws ConfigurationException {
        ...
        if (state == null)
            throw new RuntimeException(
                    String.format("Cannot replace_address %s because it doesn't exist in gossip", replaceAddress));
        ...
        try {
            ...
            if (state.isEmptyWithoutStatus() && REPLACEMENT_ALLOW_EMPTY.getBoolean()) {
                logger.warn("Gossip state not present for replacing node {}. Adding temporary entry to continue.",
                        replaceAddress);
        ...
        if (isReplacingSameAddress()) {
            ...
            SystemKeyspace.setLocalHostId(localHostId); // use the replacee's host Id as our own so we receive hints,
                                                        // etc
        }
    ...
    public synchronized void checkForEndpointCollision(UUID localHostId, Set<InetAddressAndPort> peers)
        ...
        // If not bootstrapping, compare the host id for this endpoint learned from
        // gossip (if any) with the local
        // one, which was either read from system.local or generated at startup. If a
        // learned id is present &
        // doesn't match the local, then the node needs replacing
        if (!Gossiper.instance.isSafeForStartup(FBUtilities.getBroadcastAddressAndPort(), localHostId,
        ...
        if (shouldBootstrap() && useStrictConsistency && !allowSimultaneousMoves()) {
            for (Map.Entry<InetAddressAndPort, EndpointState> entry : epStates.entrySet()) {
                ...
                if (entry.getKey().equals(FBUtilities.getBroadcastAddressAndPort())
                        || (entry.getValue().getApplicationState(ApplicationState.STATUS_WITH_PORT) == null
    ...
    public synchronized void initServer(int schemaTimeoutMillis, int ringTimeoutMillis) throws ConfigurationException {
        ...
        if (joinRing) {
            ...
            if (!tokens.isEmpty()) {
                tokenMetadata.updateNormalTokens(tokens, FBUtilities.getBroadcastAddressAndPort());
                // order is important here, the gossiper can fire in between adding these two
                // states. It's ok to send TOKENS without STATUS, but *not* vice versa.
                List<Pair<ApplicationState, VersionedValue>> states = new ArrayList<Pair<ApplicationState, VersionedValue>>();
                states.add(Pair.create(ApplicationState.TOKENS, valueFactory.tokens(tokens)));
    ...
    public void populateTokenMetadata() {
        if (Boolean.parseBoolean(System.getProperty("cassandra.load_ring_state", "true"))) {
            populatePeerTokenMetadata();
            // if we have not completed bootstrapping, we should not add ourselves as a
            // normal token
            if (!shouldBootstrap())
                tokenMetadata.updateNormalTokens(SystemKeyspace.getSavedTokens(),
                        FBUtilities.getBroadcastAddressAndPort());
     ...
     * prevent hanging (OOM for instance)
     */
        ...
        if (!joined) {
            ...
            if (replacing) {
                ...
                if (!shouldBootstrap()) {
                    ...
                    logger.warn(
                            "Writes will not be forwarded to this node during replacement because it has the same address as "
                                    +
                ...
                if (SystemKeyspace.bootstrapComplete()) {
                    ...
                    // tokens are only ever saved to system.local after bootstrap has completed and
                    // we're joining the ring,
            ...
            Gossiper.instance.register(this);
            Gossiper.instance.start(SystemKeyspace.incrementAndGetGeneration(), appStates); // needed for node-ring
                                                                                            // gathering.
            gossipActive = true;
    ...
    public void waitForSchema(long schemaTimeoutMillis, long ringTimeoutMillis) {
        ...
        if (!Schema.instance.waitUntilReady(java.time.Duration.ofMillis(schemaTimeoutMillis)))
            throw new IllegalStateException(
                    "Could not achieve schema readiness in " + java.time.Duration.ofMillis(schemaTimeoutMillis));
    }
    ...
    public void joinTokenRing(boolean finishJoiningRing,
        ...
        //
        // The seed check allows us to skip the RING_DELAY sleep for the single-node
        // cluster case,
        // which is useful for both new users and testing.
        //
        // We attempted to replace this with a schema-presence check, but you need a
        // meaningful sleep
        // to get schema info from gossip which defeats the purpose. See CASSANDRA-4427
        ...
        if (shouldBootstrap) {
            ...
            if (bootstrapTokens.isEmpty()) {
                bootstrapTokens = BootStrapper.getBootstrapTokens(tokenMetadata,
                        FBUtilities.getBroadcastAddressAndPort(), schemaTimeoutMillis, ringTimeoutMillis);
            } else {
                if (bootstrapTokens.size() != DatabaseDescriptor.getNumTokens())
                    throw new ConfigurationException("Cannot change the number of tokens from " + bootstrapTokens.size()
                            + " to " + DatabaseDescriptor.getNumTokens());
                else
    ...
    private synchronized void joinRing(boolean resumedBootstrap) throws IOException {
        if (!joined) {
            ...
            // node can join the ring even if isBootstrapMode is true which should not
            // happen
            if (!isBootstrapMode()) {
        ...
        if (!authSetupCalled.getAndSet(true)) {
            if (setUpSchema) {
                Schema.instance.transform(
                        SchemaTransformations.updateSystemKeyspace(AuthKeyspace.metadata(), AuthKeyspace.GENERATION));
            }
    ...
    public void setUpDistributedSystemKeyspaces() {
        Schema.instance.transform(
                SchemaTransformations.updateSystemKeyspace(TraceKeyspace.metadata(), TraceKeyspace.GENERATION));
        Schema.instance.transform(SchemaTransformations.updateSystemKeyspace(SystemDistributedKeyspace.metadata(),
        ...
        try {
            ...
            if (sourceDc != null)
                streamer.addSourceFilter(
                        new RangeStreamer.SingleDatacenterFilter(DatabaseDescriptor.getEndpointSnitch(), sourceDc));
            ...
            // This is used exclusively through JMX, so log the full trace but only throw a
            // simple RTE
    ...
    public void setInternodeStreamingTcpUserTimeoutInMS(int value) {
        Preconditions.checkArgument(value >= 0,
                "TCP user timeout cannot be negative for internode streaming connection. Got %s", value);
    ...
    public void setStreamThroughputMbitPerSec(int value) {
        ...
        logger.info(
                "setstreamthroughput: throttle set to {}{} megabits per second (was approximately {} megabits per second)",
    ...
    public void setStreamThroughputMebibytesPerSec(int value) {
        ...
        StreamManager.StreamRateLimiter.updateThroughput();
        logger.info("setstreamthroughput: throttle set to {}{} MiB/s (was {} MiB/s)", value,
    ...
    public void setInterDCStreamThroughputMbitPerSec(int value) {
        ...
        logger.info("setinterdcstreamthroughput: throttle set to {}{} megabits per second (was {} megabits per second)",
                value, value <= 0 ? " (unlimited)" : "", oldValue);
    }
    ...
    public void setInterDCStreamThroughputMebibytesPerSec(int value) {
        ...
        StreamManager.StreamRateLimiter.updateInterDCThroughput();
        logger.info("setinterdcstreamthroughput: throttle set to {}{} MiB/s (was {} MiB/s)", value,
                value <= 0 ? " (unlimited)" : "", oldValue);
    }
    ...
    public void setEntireSSTableInterDCStreamThroughputMebibytesPerSec(int value) {
        ...
        logger.info("setinterdcstreamthroughput (entire SSTable): throttle set to {}{} MiB/s (was {} MiB/s)", value,
                value <= 0 ? " (unlimited)" : "", oldValue);
    }
    ...
    public void setConcurrentValidators(int value) {
        ...
        if (value <= 0) {
            logger.info("Using default value of concurrent_compactors ({}) for concurrent_validations",
                    concurrentCompactors);
    ...
    public Collection<InetAddressAndPort> prepareForBootstrap(long schemaTimeoutMillis, long ringTimeoutMillis) {
        ...
        if (useStrictConsistency && !allowSimultaneousMoves() &&
            ...
            String movingTokens = StringUtils
                    .join(tokenMetadata.getMovingEndpoints().stream().map(e -> e.right).toArray(), ',');
            throw new UnsupportedOperationException(String.format(
                    "Other bootstrapping/leaving/moving nodes detected, cannot bootstrap while cassandra.consistent.rangemovement is true. Nodes detected, bootstrapping: %s; leaving: %s; moving: %s;",
        ...
        if (!replacing) {
            ...
            bootstrapTokens = BootStrapper.getBootstrapTokens(tokenMetadata, FBUtilities.getBroadcastAddressAndPort(),
    ...
    public boolean bootstrap(final Collection<Token> tokens, long bootstrapTimeoutMillis) {
        ...
        SystemKeyspace.updateTokens(tokens); // DON'T use setToken, that makes us part of the ring locally which is
                                             // incorrect until we are done bootstrapping
        ...
        if (!replacing || !isReplacingSameAddress()) {
            ...
            states.add(Pair.create(ApplicationState.STATUS_WITH_PORT,
                    replacing ? valueFactory.bootReplacingWithPort(DatabaseDescriptor.getReplaceAddress())
                            : valueFactory.bootstrapping(tokens)));
            states.add(Pair.create(ApplicationState.STATUS,
                    replacing ? valueFactory.bootReplacing(DatabaseDescriptor.getReplaceAddress().getAddress())
                            : valueFactory.bootstrapping(tokens)));
            Gossiper.instance.addLocalApplicationStates(states);
    ...
    public boolean resumeBootstrap() {
        if (isBootstrapMode && SystemKeyspace.bootstrapInProgress()) {
            ...
            BootStrapper bootstrapper = new BootStrapper(FBUtilities.getBroadcastAddressAndPort(), tokens,
                    tokenMetadata);
            bootstrapper.addProgressListener(progressSupport);
            Future<StreamState> bootstrapStream = bootstrapper.bootstrap(streamStateStore,
                    useStrictConsistency && !replacing); // handles token update
    ...
    public Map<String, List<Integer>> getConcurrency(List<String> stageNames) {
        Stream<Stage> stageStream = stageNames.isEmpty() ? stream(Stage.values())
    ...
    public String getNativeaddress(InetAddressAndPort endpoint, boolean withPort) {
        if (endpoint.equals(FBUtilities.getBroadcastAddressAndPort()))
            return FBUtilities.getBroadcastNativeAddressAndPort().getHostAddress(withPort);
                ...
                .getApplicationState(ApplicationState.NATIVE_ADDRESS_AND_PORT) != null) {
            try {
                InetAddressAndPort address = InetAddressAndPort
                        .getByName(Gossiper.instance.getEndpointStateForEndpoint(endpoint)
                                .getApplicationState(ApplicationState.NATIVE_ADDRESS_AND_PORT).value);
            ...
            // If RPC_ADDRESS present in gossip for this endpoint use it. This is expected
            // for 3.x nodes.
            if (Gossiper.instance.getEndpointStateForEndpoint(endpoint)
                ...
                ipAddress = Gossiper.instance.getEndpointStateForEndpoint(endpoint)
                        .getApplicationState(ApplicationState.RPC_ADDRESS).value;
            } else {
            ...
            try {
                InetAddressAndPort address = InetAddressAndPort.getByNameOverrideDefaults(ipAddress,
    ...
    private Map<List<String>, List<String>> getPendingRangeToEndpointMap(String keyspace, boolean withPort) {
        // some people just want to get a visual representation of things. Allow null
        ...
        Map<List<String>, List<String>> map = new HashMap<>();
        for (Map.Entry<Range<Token>, EndpointsForRange> entry : tokenMetadata.getPendingRangesMM(keyspace).asMap()
                .entrySet()) {
            map.put(entry.getKey().asList(), Replicas.stringify(entry.getValue(), withPort));
        }
        return map;
     ...
     *
     * @throws InvalidRequestException if there is no ring information available
     *                                 about keyspace
     */
    ...
    /**
     * The same as {@code describeRing(String)} but considers only the part of the
     * ring formed by nodes in the local DC.
     */
    public List<TokenRange> describeLocalRing(String keyspace) throws InvalidRequestException {
        return describeRing(keyspace, true, false);
    }

    private List<TokenRange> describeRing(String keyspace, boolean includeOnlyLocalDC, boolean withPort)
        ...
        EndpointsByRange rangeToAddressMap = includeOnlyLocalDC
                ? getRangeToAddressMapInLocalDC(keyspace)
                : getRangeToAddressMap(keyspace);

        for (Map.Entry<Range<Token>, EndpointsForRange> entry : rangeToAddressMap.entrySet())
            ranges.add(TokenRange.create(tf, entry.getKey(), ImmutableList.copyOf(entry.getValue().endpoints()),
                    withPort));
    ...
    private Map<String, String> getEndpointToHostId(boolean withPort) {
        ...
        for (Map.Entry<InetAddressAndPort, UUID> entry : getTokenMetadata().getEndpointToHostIdMapForReading()
                .entrySet())
    ...
    /*
     * Handle the reception of a new particular ApplicationState for a particular
     * endpoint. Note that the value of the
     * ApplicationState has not necessarily "changed" since the last known value, if
     * we already received the same update
     * from somewhere else.
     *
     * onChange only ever sees one ApplicationState piece change at a time (even if
     * many ApplicationState updates were
     * received at the same time), so we perform a kind of state machine here. We
     * are concerned with two events: knowing
     * the token associated with an endpoint, and knowing its operation mode. Nodes
     * can start in either bootstrap or
     * normal mode, and from bootstrap mode can change mode to normal. A node in
     * bootstrap mode needs to have
     * pendingranges set in TokenMetadata; a node in normal mode should instead be
     ...
     * you should never bootstrap a new node during a removenode, decommission or
     * move.
     */
     ...
     * status to not ready, and drain is called by the shutdown hook, it may be that
     * value is false
     * and there is no local endpoint state. In this case it's OK to just do
     * nothing. Therefore,
     * we assert that the local endpoint state is not null only when value is true.
     *
     * @param value - true indicates that RPC is ready, false indicates the
     *              opposite.
     */
    ...
    private Collection<Token> getTokensFor(InetAddressAndPort endpoint) {
        try {
            ...
            return TokenSerializer.deserialize(tokenMetadata.partitioner,
    ...
    private void handleStateBootstrap(InetAddressAndPort endpoint) {
        ...
        // if this node is present in token metadata, either we have missed intermediate
        // states
        ...
        if (tokenMetadata.isMember(endpoint)) {
            ...
            // leave operation and rebootstrapping is relatively short, so the latter is
            // quite
    ...
    private void updateTokenMetadata(InetAddressAndPort endpoint, Iterable<Token> tokens,
        ...
        for (final Token token : tokens) {
            // we don't want to update if this node is responsible for the token and it has
            // a later startup time than endpoint.
            ...
            if (currentOwner == null) {
                ...
                // set state back to normal, since the node may have tried to leave, but failed
                // and is now back up
                tokensToUpdateInMetadata.add(token);
                tokensToUpdateInSystemKeyspace.add(token);
            }
            // Note: in test scenarios, there may not be any delta between the heartbeat
            // generations of the old
            // and new nodes, so we first check whether the new endpoint is marked as a
            // replacement for the old.
            else if (endpoint.equals(tokenMetadata.getReplacementNode(currentOwner).orElse(null))
                    || Gossiper.instance.compareEndpointStartup(endpoint, currentOwner) > 0) {
                tokensToUpdateInMetadata.add(token);
                tokensToUpdateInSystemKeyspace.add(token);

                // currentOwner is no longer current, endpoint is. Keep track of these moves,
                // because when
                // a host no longer has any tokens, we'll want to remove it.
                Multimap<InetAddressAndPort, Token> epToTokenCopy = getTokenMetadata()
                        .getEndpointToTokenMapForReading();
                epToTokenCopy.get(currentOwner).remove(token);
                if (epToTokenCopy.get(currentOwner).isEmpty())
                    endpointsToRemove.add(currentOwner);

                logger.info("Nodes {} and {} have the same token {}. {} is the new owner", endpoint, currentOwner,
                        token, endpoint);
            } else {
                logger.info("Nodes {} and {} have the same token {}.  Ignoring {}", endpoint, currentOwner, token,
                        endpoint);
            }
        }
    ...
    public boolean isReplacingSameHostAddressAndHostId(UUID hostId) {
        try {
            ...
            // bootstrap completes, when it completes and advertises NORMAL state to other
            // nodes, they will be unable
            // to resolve it to an InetAddress unless it happens to be cached. This could
            // happen on nodes
            // storing large amounts of data or with long index rebuild times or if new
            // instances have been added
            // to the cluster through expansion or additional host replacement.
            //
            // The original host replacement must have been able to resolve the replacing
            // address on startup
            // when setting StorageService.replacing, so if it is impossible to resolve now
            // it is probably
            // decommissioned and did not have the same IP address or host id. Allow the
            // handleStateNormal
            // handling to proceed, otherwise gossip state will be inconistent with some
            // nodes believing the
            // replacement host to be normal, and nodes unable to resolve the hostname will
            // be left in JOINING.
            if (ex.getCause() != null && ex.getCause().getClass() == UnknownHostException.class) {
                logger.info(
                        "Suppressed exception while checking isReplacingSameHostAddressAndHostId({}). Original host was probably decommissioned. ({})",
    ...
    /**
     * Handle node move to normal state. That is, node is entering token ring and
     * participating
     * in reads.
     *
     * @param endpoint node
    ...
    private void handleStateLeaving(InetAddressAndPort endpoint) {
        ...
        // at this point the endpoint is certainly a member with this token, so let's
        // proceed
        // normally
    ...
    private void handleStateRemoving(InetAddressAndPort endpoint, String[] pieces) {
        ...
        if (tokenMetadata.isMember(endpoint)) {
            ...
            if (VersionedValue.REMOVED_TOKEN.equals(state)) {
                ...
                // find the endpoint coordinating this removal that we need to notify when we're
                // done
                String[] coordinator = splitValue(Gossiper.instance.getEndpointStateForEndpoint(endpoint)
    ...
    private void excise(Collection<Token> tokens, InetAddressAndPort endpoint) {
        ...
        if (hostId != null && tokenMetadata.isMember(endpoint)) {
            // enough time for writes to expire and MessagingService timeout reporter
            // callback to fire, which is where
            // hints are mostly written from - using getMinRpcTimeout() / 2 for the
            // interval.
     ...
     */
    private Multimap<InetAddressAndPort, FetchReplica> getNewSourceReplicas(String keyspaceName,
            Set<LeavingReplica> leavingReplicas) {
        InetAddressAndPort myAddress = FBUtilities.getBroadcastAddressAndPort();
        EndpointsByRange rangeReplicas = Keyspace.open(keyspaceName).getReplicationStrategy()
        ...
        for (LeavingReplica leaver : leavingReplicas) {
            ...
            boolean transientToFull = myCurrentReplica.isPresent() && myCurrentReplica.get().isTransient()
                    && ourReplica.isFull();
            assert !sortedPossibleReplicas.endpoints().contains(myAddress) || transientToFull
                    : String.format("My address %s, sortedPossibleReplicas %s, myCurrentReplica %s, myNewReplica %s",
                            myAddress, sortedPossibleReplicas, myCurrentReplica, ourReplica);

            // Originally this didn't log if it couldn't restore replication and that seems
            // wrong
    ...
    private void restoreReplicaCount(InetAddressAndPort endpoint, final InetAddressAndPort notifyEndpoint) {
        ...
        for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces().names()) {
            ...
            EndpointsByReplica changedReplicas = getChangedReplicasForLeaving(keyspaceName, endpoint, tokenMetadata,
            ...
            for (Map.Entry<Replica, Replica> entry : changedReplicas.flattenEntries()) {
                ...
                if (replica.endpoint().equals(myAddress)) {
                    ...
                    // but it's probably not a lot and it probably makes things a hair more
                    // resilient to people
        ...
        replicasToFetch.forEach((keyspaceName, sources) -> {
            ...
            sources.asMap().forEach((sourceAddress, fetchReplicas) -> {
                ...
                // Remember whether this node is providing the full or transient replicas for
                // this range. We are going
                // to pass streaming the local instance of Replica for the range which doesn't
                // tell us anything about the source
                // By encoding it as two separate sets we retain this information about the
                // source.
                ...
                RangesAtEndpoint transientReplicas = fetchReplicas.stream()
                        ...
                        .collect(RangesAtEndpoint.collector(myAddress));
                if (logger.isDebugEnabled())
                    logger.debug("Requesting from {} full replicas {} transient replicas {}", sourceAddress,
                            StringUtils.join(full, ", "), StringUtils.join(transientReplicas, ", "));
     ...
     * Graceful decomission should never lose data and it's going to be important
     * that transient data
     * is streamed to at least one other node from this one for each range.
     *
     * For ranges this node replicates its removal should cause a new replica to be
     * selected either as transient or full
     * for every range. So I believe the current code doesn't have to do anything
     * special because it will engage in streaming
     * for every range it replicates to at least one other node and that should
     * propagate the transient data that was here.
     * When I graphed this out on paper the result of removal looked correct and
     * there are no issues such as
    ...
    static EndpointsByReplica getChangedReplicasForLeaving(String keyspaceName, InetAddressAndPort endpoint,
            TokenMetadata tokenMetadata, AbstractReplicationStrategy strat) {
        ...
        // endpoint might or might not be 'leaving'. If it was not leaving (that is,
        // removenode
        ...
        for (Replica replica : replicas) {
            ...
            newReplicaEndpoints = newReplicaEndpoints.filter(newReplica -> {
                Optional<Replica> currentReplicaOptional = tryFind(currentReplicaEndpoints.get(replica),
                        currentReplica -> newReplica.endpoint().equals(currentReplica.endpoint())).toJavaUtil();
                // If it is newly replicating then yes we must do something to get the data
                // there
    ...
    public void onDead(InetAddressAndPort endpoint, EndpointState state) {
        // interrupt any outbound connection; if the node is failing and we cannot
        // reconnect,
        // this will rapidly lower the number of bytes we are willing to queue to the
        // node
    ...
    public void onRestart(InetAddressAndPort endpoint, EndpointState state) {
        // If we have restarted before the node was even marked down, we need to reset
        // the connection pool
        if (state.isAlive())
            onDead(endpoint, state);

        // Then, the node may have been upgraded and changed its messaging protocol
        // version. If so, we
    ...
    private Map<String, String> getLoadMap(boolean withPort) {
        ...
        // gossiper doesn't see its own updates, so we need to special-case the local
        // node
    ...
    public int scrub(boolean disableSnapshot, boolean skipCorrupted, String keyspaceName, String... tables)
            throws IOException, ExecutionException, InterruptedException {
    ...
    public int verify(boolean extendedVerify, boolean checkVersion, boolean diskFailurePolicy,
            boolean mutateRepairStatus, boolean checkOwnsTokens, boolean quick, String keyspaceName,
    ...
    public int rewriteSSTables(String keyspaceName,
        ...
        for (ColumnFamilyStore cfStore : getValidColumnFamilies(true, true, keyspaceName, tableNames)) {
            CompactionManager.AllSSTableOpStatus oneStatus = cfStore.sstablesRewrite(skipIfCurrentVersion,
    ...
    public int relocateSSTables(int jobs, String keyspaceName, String... columnFamilies)
            throws IOException, ExecutionException, InterruptedException {
        CompactionManager.AllSSTableOpStatus status = CompactionManager.AllSSTableOpStatus.SUCCESSFUL;
    ...
    public int garbageCollect(String tombstoneOptionString, int jobs, String keyspaceName, String... columnFamilies)
            throws IOException, ExecutionException, InterruptedException {
    ...
    /**
     * Takes the snapshot of a multiple column family from different keyspaces. A
     * snapshot name must be specified.
     *
     * @param tag
    ...
    public void takeSnapshot(String tag, Map<String, String> options, String... entities) throws IOException {
        DurationSpec.IntSecondsBound ttl = options.containsKey("ttl")
    ...
    @Override
    public void forceKeyspaceCompactionForPartitionKey(String keyspaceName, String partitionKey, String... tableNames)
            throws IOException, ExecutionException, InterruptedException {
        ...
        for (ColumnFamilyStore cfStore : getValidColumnFamilies(true, false, keyspaceName, tableNames)) {
            try {
                ...
                // JMX can not handle exceptions defined outside of java.* and javax.*, so safer
                // to rewrite the exception
                IllegalArgumentException exception = new IllegalArgumentException(
                        String.format("Unable to parse partition key '%s' for table %s; %s", partitionKey,
                                cfStore.metadata, e.getMessage()));
     ...
     * @param keyspaceNames the names of the keyspaces to snapshot; empty means
     *                      "all."
     */
     ...
     *                  the tag given to the snapshot; may not be null or empty
     * @param tableList
     *                  list of tables from different keyspace in the form of
     *                  ks1.cf1 ks2.cf2
     */
     ...
     *                      "all."
     */
    private void takeSnapshot(String tag, boolean skipFlush, DurationSpec.IntSecondsBound ttl, String... keyspaceNames)
            throws IOException {
     ...
     *                  ks1.cf1 ks2.cf2
     */
    private void takeMultipleTableSnapshot(String tag, boolean skipFlush, DurationSpec.IntSecondsBound ttl,
            String... tableList)
            throws IOException {
        ...
        for (String table : tableList) {
            ...
            if (splittedString.length == 2) {
                ...
                // As there can be multiple column family from same keyspace check if snapshot
                // exist for that specific
                ...
                // Add Keyspace columnfamily to map in order to support atomicity for snapshot
                // process.
                // So no snapshot should happen if any one of the above conditions fail for any
                // keyspace or columnfamily
    ...
    public void clearSnapshot(String tag, String... keyspaceNames) throws IOException {
        ...
        for (String dataDir : DatabaseDescriptor.getAllDataFileLocations()) {
            for (String keyspaceDir : new File(dataDir).tryListNames()) {
                // Only add a ks if it has been specified as a param, assuming params were
                // actually provided.
     ...
     */
    public Iterable<ColumnFamilyStore> getValidColumnFamilies(boolean allowIndexes, boolean autoAddIndexes,
            String keyspaceName, String... cfNames) throws IOException {
        Keyspace keyspace = getValidKeyspace(keyspaceName);
    ...
    private FutureTask<Object> createRepairTask(final int cmd, final String keyspace, final RepairOption options,
        ...
        Set<String> existingDatacenters = tokenMetadata.cloneOnlyTokenMap().getTopology().getDatacenterEndpoints()
                .keys().elementSet();
    ...
    private void repairPaxosForTopologyChange(String reason) {
        if (getSkipPaxosRepairOnTopologyChange() || !Paxos.useV2()) {
            logger.info(
                    "skipping paxos repair for {}. skip_paxos_repair_on_topology_change is set, or v2 paxos variant is not being used",
                    reason);
            return;
        }
     ...
     */
    public Collection<Range<Token>> getPrimaryRangeForEndpointWithinDC(String keyspace,
    ...
    public List<InetAddressAndPort> getReplicaNodesWithPortFromTokenForDegradeRead(String keyspaceName, Token token) {
        ...
        logger.debug("rymDebug: all hosts number is ({}), hosts are ({})", allHosts.size(), allHosts);
     ...
     */
    public List<Pair<Range<Token>, Long>> getSplits(String keyspaceName, String cfName, Range<Token> range,
            int keysPerSplit) {
        ...
        // splitCount should be much smaller than number of key samples, to avoid huge
        // sampling error
    ...
    public void decommission(boolean force) throws InterruptedException {
        ...
        if (operationMode != Mode.LEAVING) {
            ...
            if (operationMode != Mode.NORMAL)
                throw new UnsupportedOperationException(
                        "Node in " + operationMode + " state; wait for status to become normal or restart");
        }
        ...
        try {
            ...
            if (operationMode != Mode.LEAVING) // If we're already decommissioning there is no point checking RF/pending
                                               // ranges
            {
                int rf, numNodes;
                for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces().names()) {
                    if (!force) {
                        ...
                        if (keyspace.getReplicationStrategy() instanceof NetworkTopologyStrategy) {
                            NetworkTopologyStrategy strategy = (NetworkTopologyStrategy) keyspace
                    ...
                    if (tokenMetadata.getPendingRanges(keyspaceName, FBUtilities.getBroadcastAddressAndPort())
                            .size() > 0)
                        throw new UnsupportedOperationException(
    ...
    public Supplier<Future<StreamState>> prepareUnbootstrapStreaming() {
        ...
        for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces().names()) {
            EndpointsByReplica rangesMM = getChangedReplicasForLeaving(keyspaceName,
    ...
    private void unbootstrap(Runnable onFinish) throws ExecutionException, InterruptedException {
        ...
        // Start with BatchLog replay, which may create hints but no writes since this
        // is no longer a valid endpoint.
    ...
    /**
     * Find the best target to stream hints to. Currently the closest peer according
     * to the snitch
     */
    private UUID getPreferredHintsStreamTarget() {
        Set<InetAddressAndPort> endpoints = StorageService.instance.getTokenMetadata().cloneAfterAllLeft()
                .getAllEndpoints();
        ...
        if (candidates.isEmpty()) {
            ...
            candidates = DatabaseDescriptor.getEndpointSnitch()
                    .sortedByProximity(FBUtilities.getBroadcastAddressAndPort(), candidates);
     ...
     *
     * @param newToken new token to boot to, or if null, find balanced token to boot
     *                 to
     *
    ...
    private void move(Token newToken) throws IOException {
        ...
        setMode(Mode.MOVING,
                String.format("Moving %s from %s to %s.", localAddress, getLocalTokens().iterator().next(), newToken),
                true);

        setMode(Mode.MOVING, String.format("Sleeping %s ms before start streaming/fetching ranges", RING_DELAY_MILLIS),
                true);
        Uninterruptibles.sleepUninterruptibly(RING_DELAY_MILLIS, MILLISECONDS);
        ...
        if (relocator.streamsNeeded()) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Interrupted while waiting for stream/fetch ranges to finish: " + e.getMessage());
            }
        } else {
     ...
     * first, this is a last resort measure. No further attempt will be made to
     * restore replicas.
     */
    ...
    public void removeNode(String hostIdString) {
        ...
        for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces().names()) {
            ...
            EndpointsByReplica changedRanges = getChangedReplicasForLeaving(keyspaceName, endpoint, tokenMetadata,
                    Keyspace.open(keyspaceName).getReplicationStrategy());
            IFailureDetector failureDetector = FailureDetector.instance;
    ...
    public void confirmReplication(InetAddressAndPort node) {
        // replicatingNodes can be empty in the case where this node used to be a
        // removal coordinator,
        // but restarted before all 'replication finished' messages arrived. In that
        // case, we'll
    ...
    protected synchronized void drain(boolean isFinalShutdown)
        ...
        if (preShutdownHookThrowable != null)
            logger.error("Attempting to continue draining after pre-shutdown hooks returned exception",
        ...
        try {
            ...
            try {
                /*
                 * not clear this is reasonable time, but propagated from prior embedded
                 * behaviour
                 */
            ...
            // which is currently scheduled on the mutation stage. So shut down
            // MessagingService
            ...
            // count CFs first, since forceFlush could block for the flushWriter to get a
            // queue slot empty
            totalCFs = 0;
            ...
            for (Future f : flushes) {
                try {
                    ...
                    JVMStabilityInspector.inspectThrowable(t);
                    // don't let this stop us from shutting down the commitlog and other thread
                    // pools
            ...
            // Flush the system tables after all other tables are flushed, just in case
            ...
            // system tables (for example compactions can obsolete sstables and the tidiers
            // in SSTableReader update
            ...
            try {
                // we are not shutting down ScheduledExecutors#scheduledFastTasks to be still
                // able to progress time
                // fast-tasks executor is shut down in StorageService's shutdown hook added to
                // Runtime
    ...
    /**
     * Add a runnable which will be called before shut down or drain. This is useful
     * for other
     * applications running in the same JVM which may want to shut down first rather
     * than time
     * out attempting to use Cassandra calls which will no longer work.
     * 
     * @param hook: the code to run
     * @return true on success, false if Cassandra is already shutting down, in
     *         which case the runnable
     *         has NOT been added.
     */
    ...
    /**
     * Add a runnable which will be called after shutdown or drain. This is useful
     * for other applications
     * running in the same JVM that Cassandra needs to work and should shut down
     * later.
     * 
     * @param hook: the code to run
     * @return true on success, false if Cassandra is already shutting down, in
     *         which case the runnable has NOT been
     *         added.
     */
    ...
    /**
     * Some services are shutdown during draining and we should not attempt to start
     * them again.
     *
     * @param service - the name of the service we are trying to start.
     * @throws IllegalStateException - an exception that nodetool is able to convert
     *                               into a message to display to the user
     */
    synchronized void checkServiceAllowedToStart(String service) {
        ...
        if (isShutdown()) // do not rely on operationMode in case it gets changed to decomissioned or
                          // other
            throw new IllegalStateException(String.format("Unable to start %s because the node was drained.", service));

        if (!isNormal() && joinRing) // if the node is not joining the ring, it is gossipping-only member which is in
                                     // STARTING state forever
            throw new IllegalStateException(
                    String.format("Unable to start %s because the node is not in the normal state.", service));
    }
     ...
     * is DC aware then ownership will be
     * calculated per dc, i.e. each DC will have total ring ownership divided
     * amongst its nodes. Without replication
     * total ownership will be a multiple of the number of DC's and this value will
     * then go up within each DC depending
    ...
    private LinkedHashMap<InetAddressAndPort, Float> getEffectiveOwnership(String keyspace) {
        ...
        if (keyspace != null) {
            ...
            if (userKeyspaces.size() > 0) {
                ...
                for (String keyspaceName : userKeyspaces) {
                    if (!Schema.instance.getKeyspaceInstance(keyspaceName).getReplicationStrategy()
            ...
            Keyspace keyspaceInstance = Schema.instance.getKeyspaceInstance(keyspace);
            if (keyspaceInstance == null)
                throw new IllegalStateException("The node does not have " + keyspace
                        + " yet, probably still bootstrapping. Effective ownership information is meaningless.");
            strategy = keyspaceInstance.getReplicationStrategy();
        }
        ...
        SortedMap<String, Collection<InetAddressAndPort>> sortedDcsToEndpoints = new TreeMap<>(
                metadata.getTopology().getDatacenterEndpoints().asMap());
    ...
    public LinkedHashMap<String, Float> effectiveOwnershipWithPort(String keyspace) throws IllegalStateException {
        ...
        result.entrySet().stream()
                .forEachOrdered(entry -> asStrings.put(entry.getKey().getHostAddressAndPort(), entry.getValue()));
        return asStrings;
    }
    ...
    public void updateSnitch(String epSnitchClassName, Boolean dynamic, Integer dynamicUpdateInterval,
        ...
        if (epSnitchClassName != null) {
            ...
            // need to unregister the mbean _before_ the new dynamic snitch is instantiated
            // (and implicitly initialized
            ...
            if (newSnitch instanceof DynamicEndpointSnitch) {
                logger.info(
                        "Created new dynamic snitch {} with update-interval={}, reset-interval={}, badness-threshold={}",
            ...
            if (oldSnitch instanceof DynamicEndpointSnitch) {
                logger.info(
                        "Applying config change to dynamic snitch {} with update-interval={}, reset-interval={}, badness-threshold={}",
     ...
     *
     * @param rangesToStreamByKeyspace keyspaces and data ranges with endpoints
     *                                 included for each
    ...
    private Future<StreamState> streamRanges(Map<String, EndpointsByReplica> rangesToStreamByKeyspace) {
        ...
        for (Map.Entry<String, EndpointsByReplica> entry : rangesToStreamByKeyspace.entrySet()) {
            ...
            Map<InetAddressAndPort, Set<Range<Token>>> transferredRangePerKeyspace = SystemKeyspace
                    .getTransferredRanges("Unbootstrap",
                            keyspace,
        ...
        // Vinculate StreamStateStore to current StreamPlan to update transferred
        // rangeas per StreamSession
        ...
        for (Map.Entry<String, RangesByEndpoint> entry : sessionsToStreamByKeyspace.entrySet()) {
            ...
            for (Map.Entry<InetAddressAndPort, RangesAtEndpoint> rangesEntry : replicasPerEndpoint.asMap().entrySet()) {
                ...
                // TODO each call to transferRanges re-flushes, this is potentially a lot of
                // waste
    ...
    private StreamResultFuture bulkLoadInternal(String directory) {
        ...
        SSTableLoader.Client client = new SSTableLoader.Client() {
            ...
            public void init(String keyspace) {
                ...
                try {
                    for (Map.Entry<Range<Token>, EndpointsForRange> entry : StorageService.instance
    ...
    public List<String> sampleKeyRange() // do not rename to getter - see CASSANDRA-4452 for details
    {
        ...
        for (Keyspace keyspace : Keyspace.nonLocalStrategy()) {
            for (Range<Token> range : getPrimaryRangesForEndpoint(keyspace.getName(),
                    FBUtilities.getBroadcastAddressAndPort()))
    ...
    public void enableAuditLog(String loggerName, String includedKeyspaces, String excludedKeyspaces,
            String includedCategories, String excludedCategories,
            String includedUsers, String excludedUsers) throws ConfigurationException, IllegalStateException {
        enableAuditLog(loggerName, Collections.emptyMap(), includedKeyspaces, excludedKeyspaces, includedCategories,
                excludedCategories, includedUsers, excludedUsers,
    ...
    @Deprecated
    public void enableAuditLog(String loggerName, Map<String, String> parameters, String includedKeyspaces,
            String excludedKeyspaces, String includedCategories, String excludedCategories,
            String includedUsers, String excludedUsers) throws ConfigurationException, IllegalStateException {
        enableAuditLog(loggerName, parameters, includedKeyspaces, excludedKeyspaces, includedCategories,
                excludedCategories, includedUsers, excludedUsers,
                Integer.MIN_VALUE, null, null, Long.MIN_VALUE, Integer.MIN_VALUE, null);
    }
    ...
    @Override
    public void enableFullQueryLogger(String path, String rollCycle, Boolean blocking, int maxQueueWeight,
            long maxLogSize, String archiveCommand, int maxArchiveRetries) {
        ...
        FullQueryLogger.instance.enableWithoutClean(Paths.get(path), rollCycle, blocking, maxQueueWeight, maxLogSize,
    ...
    public CompositeData getFullQueryLoggerOptions() {
        return FullQueryLoggerOptionsCompositeData
                .toCompositeData(FullQueryLogger.instance.getFullQueryLoggerOptions());
    }
