...
schemes=("elect-pdm")
workloads=("workloadReadBreak" "workloadScanBreak")
types=("degraded")
waitFlush="true"
restart="true"
threadsNumber=1
...
function flush {
    ...
    # Modify playbook
    sed -i "s/\(expName: \)".*"/expName: "${ExpName}-${targetScheme}-Compact"/" playbook-flush.yaml
...
function backup {
    ...
    sed -i "s/DATAPATH/Exp7-Recovery-10000000/g" playbook-backup.yaml
...
function startup {
    ...
    sed -i "s/DATAPATH/Exp7-Recovery-10000000/g" playbook-startup.yaml
...
function run {
    ...
    for workload in "${workloads[@]}"; do
        ...
        sed -i "s/trace:.*$/trace: on/" playbook-run.yaml
...
for scheme in "${schemes[@]}"; do
    ...
    if [[ ${scheme} == *"elect"* ]]; then
        waitTime=$(echo "scale=2; ($dataSizeOnEachNode * 1024 / 4 / 2 / ($concurrentEC / 2)) * 120 + $dataSizeOnEachNode * 500" | bc)
        maxLevelCount
