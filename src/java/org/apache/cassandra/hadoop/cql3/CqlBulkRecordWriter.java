...
public class CqlBulkRecordWriter extends RecordWriter<Object, List<ByteBuffer>>
    ...
    protected String getOutputLocation() throws IOException {
        ...
        if (dir == null)
            throw new IOException("Output directory not defined, if hadoop is not setting java.io.tmpdir then define "
                    + OUTPUT_LOCATION);
        return dir;
    }
    ...
    private void prepareWriter() throws IOException {
        ...
        if (loader == null) {
            ...
            externalClient.setTableMetadata(
                    TableMetadataRef.forOfflineTools(CreateTableStatement.parse(schema, keyspace).build()));
    ...
    private File getTableDirectory() throws IOException {
        File dir = new File(String.format("%s%s%s%s%s-%s", getOutputLocation(), File.pathSeparator(), keyspace,
                File.pathSeparator(), table, UUID.randomUUID().toString()));
        ...
        private static Collection<InetSocketAddress> resolveHostAddresses(Configuration conf) {
            ...
            for (String host : ConfigHelper.getOutputInitialAddress(conf).split(",")) {
                try {
                    ...
                    addresses.add(
                            new InetSocketAddress(InetAddress.getByName(hap.getHost()), hap.getPortOrDefault(port)));
                } catch (UnknownHostException e) {
