 ...
 *
 * Modifications to instances are all performed via a Function produced by the
 * static methods in this class.
 * These are composed as necessary and provided to the Tracker.apply() methods,
 * which atomically reject or
...
public class View {
    ...
    /**
     * ordinarily a list of size 1, but when preparing to flush will contain both
     * the memtable we will flush
     * and the new replacement memtable, until all outstanding write operations on
     * the old table complete.
     * The last item in the list is always the "current" memtable.
     */
    public final List<Memtable> liveMemtables;
    /**
     * contains all memtables that are no longer referenced for writing and are
     * queued for / in the process of being
    ...
    // we use a Map here so that we can easily perform identity checks as well as
    // equality checks.
    // When marking compacting, we now indicate if we expect the sstables to be
    // present (by default we do),
    // and we then check that not only are they all present in the live set, but
    // that the exact instance present is
    ...
    View(List<Memtable> liveMemtables, List<Memtable> flushingMemtables, Map<SSTableReader, SSTableReader> sstables,
            Map<SSTableReader, SSTableReader> compacting, SSTableIntervalTree intervalTree) {
    ...
    // any sstable known by this tracker in any form; we have a special method here
    // since it's only used for testing/debug
    ...
    public Iterable<SSTableReader> select(SSTableSet sstableSet) {
        switch (sstableSet) {
            ...
            case CANONICAL:
                ...
                // reason for checking if compacting contains the sstable is that if compacting
                // has an EARLY version
                // of a NORMAL sstable, we still have the canonical version of that sstable in
                // sstables.
                // note that the EARLY version is equal, but not == since it is a different
                // instance of the same sstable.
    ...
    public String toString() {
        return String.format("View(pending_count=%d, sstables=%s, compacting=%s)",
                liveMemtables.size() + flushingMemtables.size() - 1, sstables, compacting);
    }
    ...
    public static List<SSTableReader> sstablesInBounds(PartitionPosition left, PartitionPosition right,
    ...
    /**
     * @return a ViewFragment containing the sstables and memtables that may need to
     *         be merged
     ...
     * @return a ViewFragment containing the sstables and memtables that may need to
     *         be merged
     *         for rows within @param rowBounds, inclusive, according to the
     *         interval tree.
     */
    public static Function<View, Iterable<SSTableReader>> selectLive(AbstractBounds<PartitionPosition> rowBounds) {
        // Note that View.sstablesInBounds always includes it's bound while rowBounds
        // may not. This is ok however
        // because the fact we restrict the sstables returned by this function is an
        // optimization in the first
        // place and the returned sstables will (almost) never cover *exactly* rowBounds
        // anyway. It's also
        // *very* unlikely that a sstable is included *just* because we consider one of
        // the bound inclusively
    ...
    // construct a predicate to reject views that do not permit us to mark these
    // readers compacting;
    // i.e. one of them is either already compacting, has been compacted, or has
    // been replaced
                ...
                for (SSTableReader reader : readers)
                    if (view.compacting.contains(reader) || view.sstablesMap.get(reader) != reader
                        ...
                        if (reader.isReplicationTransferredToErasureCoding()) {
                            logger.debug(
                                    "rymDebug: the transferred sstable {} is already marked as compaction! The reason is view.compacting.contains? ({}), view.sstablesMap.get(reader) != reader? ({}), view.sstablesMap.get(reader) ({}) reader.isMarkedCompacted? ({})",
    ...
    public static Function<View, View> updateLiveSet(final Set<SSTableReader> remove,
    ...
    // called prior to initiating flush: add newMemtable to liveMemtables, making it
    // the latest memtable
    static Function<View, View> switchMemtable(final Memtable newMemtable) {
        return new Function<View, View>() {
            public View apply(View view) {
                List<Memtable> newLive = ImmutableList.<Memtable>builder().addAll(view.liveMemtables).add(newMemtable)
                        .build();
                assert newLive.size() == view.liveMemtables.size() + 1;
                return new View(newLive, view.flushingMemtables, view.sstablesMap, view.compactingMap,
                        view.intervalTree);
            }
        };
    }
    ...
    // called after flush: removes memtable from flushingMemtables, and inserts
    // flushed into the live sstable set
