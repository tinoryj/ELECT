...
function load {
    ...
    modifyWorkload "workloadZIPPYDB"

    sed -i "s/\(workload: \)".*"/workload: \"workloadZIPPYDB\"/" playbook-load.yaml
...
function flush {
    ...
    sed -i "s/\(workload: \)".*"/workload: \"workloadZIPPYDB\"/" playbook-flush.yaml
...
for scheme in "${schemes[@]}"; do
    ...
    waitTime=$(echo "scale=2; $dataSizeOnEachNode * 600" | bc)
    if [ "${scheme}" == "elect" ]; then
        waitTime=$(echo "scale=2; ($dataSizeOnEachNode * 1024  / 4 / 3 / ($concurrentEC / 2)) * 100 + $dataSizeOnEachNode * 600" | bc)
        maxLevelCount
