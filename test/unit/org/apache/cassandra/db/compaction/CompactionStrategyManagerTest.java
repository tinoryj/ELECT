...
public class CompactionStrategyManagerTest {
    ...
    public static void beforeClass() {
        ...
        /**
         * We use byte ordered partitioner in this test to be able to easily infer an
         * SSTable
         * disk assignment based on its generation - See
         * {@link this#getSSTableIndex(Integer[], SSTableReader)}
         */
    ...
    public void testSSTablesAssignedToCorrectCompactionStrategy(int numSSTables, int numDisks) {
        ...
        for (int delta = 1; delta <= 3; delta++) {
            ...
            logger.debug("Old boundaries: {} New boundaries: {}", Arrays.toString(previousBoundaries),
            ...
            for (SSTableReader reader : cfs.getLiveSSTables()) {
                ...
                // Remove SSTable and check that it will be removed from the correct compaction
                // strategy
                csm.handleNotification(new SSTableDeletingNotification(reader), this);
                assertFalse(
                        ((SizeTieredCompactionStrategy) csm.compactionStrategyFor(reader)).sstables.contains(reader));
    ...
    public void testAutomaticUpgradeConcurrency() throws Exception {
        ...
        // latch to block
        // CompactionManager.BackgroundCompactionCandidate#maybeRunUpgradeTask
        // inside the currentlyBackgroundUpgrading check - with
        // max_concurrent_auto_upgrade_tasks = 1 this will make
        // sure that BackgroundCompactionCandidate#maybeRunUpgradeTask returns false
        // until the latch has been counted down
        ...
        CompactionManager.BackgroundCompactionCandidate r = CompactionManager.instance
                .getBackgroundCompactionCandidate(mock);
        CompactionStrategyManager mgr = mock.getCompactionStrategyManager();
        // basic idea is that we start a thread which will be able to get in to the
        // currentlyBackgroundUpgrading-guarded
        // code in CompactionManager, then we try to run a bunch more of the upgrade
        // tasks which should return false
        // due to the currentlyBackgroundUpgrading count being >=
        // max_concurrent_auto_upgrade_tasks
    ...
    public void testAutomaticUpgradeConcurrency2() throws Exception {
        ...
        // basic idea is that we start 2 threads who will be able to get in to the
        // currentlyBackgroundUpgrading-guarded
        // code in CompactionManager, then we try to run a bunch more of the upgrade
        // task which should return false
        // due to the currentlyBackgroundUpgrading count being >=
        // max_concurrent_auto_upgrade_tasks
        Thread t = new Thread(() -> r.maybeRunUpgradeTask(mgr));
        t.start();
    ...
    /**
     * Test that csm.groupSSTables correctly groups sstables by repaired status and
     * directory
     */
    @Test
    public void groupSSTables() throws Exception {
        ...
        List<GroupedSSTableContainer> grouped = csm
                .groupSSTables(Iterables.concat(transientRepairs, pendingRepair, repaired, unrepaired));
    ...
    private void verifySSTableIsAssignedToCorrectStrategy(Integer[] boundaries, CompactionStrategyManager csm,
    ...
    private Integer[] computeBoundaries(int numSSTables, int numDisks) {
        ...
        result[numDisks - 1] = numSSTables; // make last boundary alwyays be the number of SSTables to prevent rounding
                                            // errors
        return result;
    }
    ...
    private int getSSTableIndex(Integer[] boundaries, SSTableReader reader) {
        ...
        while (boundaries[index] <= firstKey)
            index++;
        logger.debug("Index for SSTable {} on boundary {} is {}", reader.descriptor.id, Arrays.toString(boundaries),
                index);
        return index;
    }
    ...
    class MockBoundaryManager {
        ...
        private DiskBoundaries createDiskBoundaries(ColumnFamilyStore cfs, Integer[] boundaries) {
            List<PartitionPosition> positions = Arrays.stream(boundaries)
                    .map(b -> Util.token(String.format(String.format("%04d", b))).minKeyBound())
