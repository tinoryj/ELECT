...
func() {
    ...
    file_name="${expName}-${workload}-KVNumber-${record_count}-KeySize-${}-ValueSize-${field_length}-ClientNumber-${threads}-$(date +%s)"
    ...
    bin/ycsb load cassandra-cql -p hosts=${NodesList} -p cassandra.keyspace=${keyspace} -p cassandra.tracing="false" -threads $threads -s -P workloads/"${workload}" >${PathToELECTLog}/${file_name}.log 2>&1
}
