...
public class RowIteratorMergeListener<E extends Endpoints<E>>
    ...
    /**
     * the repairs we will send to each source, suffixed by a complete repair of all
     * differences, if {@link #buildFullDiff}
     */
    ...
    // For each source, record if there is an open range to send as repair, and from
    // where.
    private final ClusteringBound<?>[] markerToRepair;

    private final ReadRepair readRepair;

    public RowIteratorMergeListener(DecoratedKey partitionKey, RegularAndStaticColumns columns, boolean isReversed,
            ReplicaPlan.ForRead<E, ?> readPlan, ReadCommand command, ReadRepair readRepair) {
        ...
        // If we are contacting any nodes we didn't read from, we are likely handling a
        // range movement.
        // In this case we need to send all differences to these nodes, as we do not
        // (with present design) know which
        // node they bootstrapped from, and so which data we need to duplicate.
        // In reality, there will be situations where we are simply sending the same
        // number of writes to different nodes
        // and in this case we could probably avoid building a full difference, and only
        // ensure each write makes it to
        // some other node, but it is probably not worth special casing this scenario.
        this.buildFullDiff = Iterables.any(writePlan.contacts().endpoints(),
                e -> !readPlan.contacts().endpoints().contains(e));
        ...
        this.diffListener = new RowDiffListener() {
            public void onPrimaryKeyLivenessInfo(int i, Clustering<?> clustering, LivenessInfo merged,
            ...
            public void onComplexDeletion(int i, Clustering<?> clustering, ColumnMetadata column, DeletionTime merged,
            ...
            private boolean isQueried(Cell<?> cell) {
                // When we read, we may have some cell that have been fetched but are not
                // selected by the user. Those cells may
                // have empty values as optimization (see CASSANDRA-10655) and hence they should
                // not be included in the read-repair.
                // This is fine since those columns are not actually requested by the user and
                // are only present for the sake of CQL
                // semantic (making sure we can always distinguish between a row that doesn't
                // exist from one that do exist but has
                /// no value for the column requested by the user) and so it won't be unexpected
                // by the user that those columns are
                // not repaired.
                ColumnMetadata column = cell.column();
                ColumnFilter filter = RowIteratorMergeListener.this.command.columnFilter();
                return column.isComplex() ? filter.fetchedCellIsQueried(column, cell.path())
    ...
    /**
     * The partition level deletion with with which source {@code i} is currently
     * repaired, or
     * {@code DeletionTime.LIVE} if the source is not repaired on the partition
     * level deletion (meaning it was
    ...
    public Row onMergedRows(Row merged, Row[] versions) {
        // If a row was shadowed post merged, it must be by a partition level or range
        // tombstone, and we handle
        // those case directly in their respective methods (in other words, it would be
        // inefficient to send a row
        // deletion as repair when we know we've already send a partition level or range
        // tombstone that covers it).
        if (merged.isEmpty())
            return merged;
    ...
    public void onMergedRangeTombstoneMarkers(RangeTombstoneMarker merged, RangeTombstoneMarker[] versions) {
        ...
        for (int i = 0; i < versions.length; i++) {
            ...
            if (merged == null) {
                ...
                // We have a close and/or open marker for a source, with nothing corresponding
                // in merged.
                // Because merged is a superset, this implies that we have a current deletion
                // (either due to an
                // early opening in merged or a partition level deletion) and that this deletion
                // will still be
                // active after that point. Further whatever deletion was open or is open by
                // this marker on the
                // source, that deletion cannot supersedes the current one.
                //
                // But while the marker deletion (before and/or after this point) cannot
                // supersede the current
                // deletion, we want to know if it's equal to it (both before and after),
                // because in that case
                // the source is up to date and we don't want to include it into repair.
                //
                // So in practice we have 2 possible cases:
                // 1) the source was up-to-date on deletion up to that point: then it won't be
                // from that point
                // on unless it's a boundary and the new opened deletion time is also equal to
                // the current
                // deletion (note that this implies the boundary has the same closing and
                // opening deletion
                // time, which should generally not happen, but can due to legacy reading code
                // not avoiding
                // this for a while, see CASSANDRA-13237).
                // 2) the source wasn't up-to-date on deletion up to that point and it may now
                // be (if it isn't
                // we just have nothing to do for that marker).
                assert !currentDeletion.isLive() : currentDeletion.toString();

                // Is the source up to date on deletion? It's up to date if it doesn't have an
                // open RT repair
                // nor an "active" partition level deletion (where "active" means that it's
                // greater or equal
                // to the current deletion: if the source has a repaired partition deletion
                // lower than the
                // current deletion, this means the current deletion is due to a previously open
                // range tombstone,
                // and if the source isn't currently repaired for that RT, then it means it's up
                // to date on it).
                ...
                if (markerToRepair[i] == null && currentDeletion.supersedes(partitionRepairDeletion)) {
                    /*
                     * Since there is an ongoing merged deletion, the only two ways we don't have an
                     * open repair for
                     * this source are that:
                     *
                     * 1) it had a range open with the same deletion as current marker, and the
                     * marker is coming from
                     * a short read protection response - repeating the open RT bound, or
                     * 2) it had a range open with the same deletion as current marker, and the
                     * marker is closing it.
                     */
                    if (!marker.isBoundary() && marker.isOpen(isReversed)) // (1)
                    {
                        assert currentDeletion.equals(marker.openDeletionTime(isReversed))
                                : String.format("currentDeletion=%s, marker=%s", currentDeletion,
                    ...
                    // and so unless it's a boundary whose opening deletion time is still equal to
                    // the current
                    // deletion (see comment above for why this can actually happen), we have to
                    // repair the source
                    // from that point on.
                    if (!(marker.isOpen(isReversed) && currentDeletion.equals(marker.openDeletionTime(isReversed))))
                        markerToRepair[i] = marker.closeBound(isReversed).invert();
                }
                // In case 2) above, we only have something to do if the source is up-to-date
                // after that point
                // (which, since the source isn't up-to-date before that point, means we're
                // opening a new deletion
                // that is equal to the current one).
                else {
                    if (markerToRepair[i] == null) {
                        // Only way we can have no open RT repair is that partition deletion that has
                        // the same timestamp
                        // as the deletion and same local deletion time. In such case, since partition
                        // deletion covers
                ...
                // We have a change of current deletion in merged (potentially to/from no
                // deletion at all).

                if (merged.isClose(isReversed)) {
                    // We're closing the merged range. If we're recorded that this should be repaird
                    // for the
                ...
                if (merged.isOpen(isReversed)) {
                    // If we're opening a new merged range (or just switching deletion), then unless
                    // the source
                    // is up to date on that deletion (note that we've updated what the source
                    // deleteion is
    ...
    private void closeOpenMarker(int i, ClusteringBound<?> close) {
        ...
        RangeTombstone rt = new RangeTombstone(Slice.make(isReversed ? close : open, isReversed ? open : close),
                currentDeletion());
    ...
    public void close() {
        ...
        for (Replica replica : writePlan.contacts()) {
            ...
            Mutation mutation = BlockingReadRepairs.createRepairMutation(update, readPlan.consistencyLevel(),
                    replica.endpoint(), false);
            if (mutation == null)
                continue;
